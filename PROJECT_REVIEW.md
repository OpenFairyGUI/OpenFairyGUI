...
## PR #155 复审修复（2026-09-26）

同步根 sharp 依赖分类及包的 Sharp 版本范围锁文件，干净临时目录冻结安装通过。Label 输入设置改为正式 inputSettings 对象，二进制与 UAM 保留完整字段、null/空字符串和输入块存在状态；未验证的 XML 表达在写入文件前明确拒绝。新增二进制独立样本、双形态往返和非法参数回归。此前模型 11/11 对应 b40d1a0 批次，不作为本次修复的模型成绩。
