...
export function createTestMovieClipJta(version: 100 | 101 | 102, options: TestMovieClipJtaOptions = {}): Uint8Array {
	...
	const byteLength =
		...
		frames.length * 12 +
		2 +
		textures.reduce((sum, texture) => sum + 4 + texture.byteLength, 0) +
	...
	offset += 2;
	bytes.set(new TextEncoder().encode('yytou'), offset);
	offset += 5;
	view.setInt32(offset, version, false);
	offset += 4;
	view.setInt8(offset, options.fps ?? 24);
	offset += 1;
	offset += 3;
	if (version === 102) {
		...
		view.setInt16(offset, 0, false);
		offset += 2;
		view.setUint16(offset, options.width ?? 0, false);
		offset += 2;
		view.setUint16(offset, options.height ?? 0, false);
		offset += 2;
	}
	view.setUint8(offset, options.speed ?? 1);
	offset += 1;
	view.setUint8(offset, options.repeatDelay ?? 0);
	offset += 1;
	view.setInt8(offset, options.swing ? 1 : 0);
	offset += 1;
	view.setInt16(offset, frames.length, false);
	offset += 2;
	for (const frame of frames) {
		view.setInt16(offset, frame.delay, false);
		offset += 2;
		view.setInt16(offset, frame.rectX, false);
		offset += 2;
		view.setInt16(offset, frame.rectY, false);
		offset += 2;
		view.setInt16(offset, frame.rectWidth, false);
		offset += 2;
		view.setInt16(offset, frame.rectHeight, false);
		offset += 2;
		view.setInt16(offset, frame.textureIndex, false);
		offset += 2;
	}
	view.setInt16(offset, textures.length, false);
	offset += 2;
	for (const texture of textures) {
		view.setInt32(offset, texture.byteLength, false);
		offset += 4;
		bytes.set(texture, offset);
		offset += texture.byteLength;
	}
	if (version === 101) {
		view.setInt16(offset, 0, false);
		offset += 2;
