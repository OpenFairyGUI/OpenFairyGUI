...
test('MCP P1 prompts are discoverable and tied to existing backend tools', async (t) => {
	await withClient(async (client) => {
		...
		const expectations: Record<OpenFairyGuiBackendPromptName, string[]> = {
			openfairygui_inspect_capabilities: ['openfairygui_backend_get_capabilities', 'artifact publish/restore', 'subscriptions'],
			...
			openfairygui_poll_runtime_state: ['openfairygui_backend_get_events', 'openfairygui_backend_get_cache_snapshot', 'openfairygui_backend_refresh_cache', 'synchronously'],
		};
		...
		for (const name of OPENFAIRYGUI_BACKEND_PROMPT_NAMES) {
			...
			for (const [tool] of text.matchAll(/openfairygui_backend_[a-z_]+/g)) t.true(toolNames.has(tool), `${name} refers to a discoverable tool: ${tool}`);
