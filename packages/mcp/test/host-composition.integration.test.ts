...
const approvalFailure = (sessionId: string, revision: number) => ({
	ok: false as const,
	error: {
		code: 'save_approval_required' as const,
		message: 'Host owner confirmation is required',
		approval: { approvalRequestId: 'host-request', sessionId, revision },
		approvalPath: '/#save-approvals' as const,
	},
});
...
test('SDK discovery includes Host tools before and after connection and follows public handles', async (t) => {
	...
	const probe = server.registerTool('host_probe', { inputSchema: z.object({}) }, async () => ({
...
for (const [method, name] of [
	['saveSession', 'openfairygui_backend_save_session'],
	['materializeSession', 'openfairygui_backend_materialize_session'],
] as const) {
	test(`${method}: declared Host failure stops writes; a grant permits one unchanged Backend result`, async (t) => {
		...
		const server = createOpenFairyGuiMcpServer({
			...
			toolPolicies: {
				[name]: {
					failureSchema,
					async beforeCall(input: Readonly<Record<string, unknown>>) {
						policyCalls++;
						if (grant === JSON.stringify(input)) {
							grant = undefined;
							return;
						}
		...
		try {
			...
			const output = z.fromJSONSchema(
				tools.find((tool) => tool.name === name)!.outputSchema as Parameters<typeof z.fromJSONSchema>[0],
			);
			...
			const other = z.fromJSONSchema(
				tools.find((tool) => tool.name === 'openfairygui_backend_get_session')!.outputSchema as Parameters<
					typeof z.fromJSONSchema
				>[0],
			);
			t.false(
				other.safeParse(pending.structuredContent).success,
				'Host branch applies only to the selected tool',
			);
			...
			t.deepEqual(retries.find((result) => !result.isError)!.structuredContent, {
				backendResult: JSON.parse(JSON.stringify(backendResults[0])),
			});
			t.is(
				(await readProjectAsUam(new NodeIO(), fixture.fairyPath)).packages[0].resources.find(
...
test('Host policy cannot bypass input validation, alter Backend input or validate Backend-owned failures', async (t) => {
	...
	runtime.getSession = (input) => {
		backendCalls++;
		return mode === 'invalid-backend' ? (approvalFailure('s', 0) as never) : getSession(input);
	};
	...
	for (mode of ['invalid-host', 'throw', 'invalid-backend']) {
		...
		t.is(
			(result.structuredContent?.backendResult as { error: { code: string } }).error.code,
	...
	t.throws(() => createOpenFairyGuiMcpServer({ runtime, toolPolicies: { misspelled_tool: policy } as never }), {
...
test('Host failures obey the same complete-response budget without invoking Backend', async (t) => {
	...
	runtime.readSessionState = () => {
		calls++;
		throw new Error('Host failure must stop Backend');
	};
	const result = await callOpenFairyGuiBackendTool(
		runtime,
