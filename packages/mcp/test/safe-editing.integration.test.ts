...
test('MCP round-trips a real image above the generic array budget through read, preview and apply', async (t) => {
	...
	const sourceBytes = new Uint8Array(
		await fs.readFile(getFixturePath('FairyGUI-unity', 'UIProject', 'assets', 'VirtualList', '8.png')),
	);
	...
	try {
		...
		const current = runtime.readResourceBytes({ sessionId, expectedRevision: 1, selector });
		assert(current.ok);
...
test('MCP byte allowances are schema-scoped, integer-only and bounded in aggregate', (t) => {
	...
	for (const value of [-1, 256, 0.5, NaN, '1'])
		t.false(isOpenFairyGuiMcpPayloadWithinBudget(payload([value]), bytePaths));
	...
	t.false(
		isOpenFairyGuiMcpPayloadWithinBudget(
			{
				operations: [
					{ sourceBytes: new Uint8Array(5 * 1024 * 1024) },
					{ sourceBytes: new Uint8Array(5 * 1024 * 1024) },
				],
			},
			bytePaths,
...
test('MCP session reads expose current unsaved state and primary bytes at one revision', async (t) => {
	...
	try {
		...
		for (const name of ['read_session_state', 'read_resource_bytes']) {
			...
			t.true(tool.annotations?.idempotentHint);
		}
		const opened = await runtime.openSession({ projectPath: fixture.fairyPath });
		assert(opened.ok);
		sessionId = opened.data.sessionId;
		const before = runtime.readSessionState({ sessionId });
		assert(before.ok);
		...
		const backend = (result.structuredContent as { backendResult: BackendResult<BackendSessionStateSnapshot> })
			.backendResult;
		assert(backend.ok);
		t.is(backend.data.revision, 1);
		t.true(backend.data.dirty);
		t.is(backend.data.lastSavedRevision, 0);
		const native = runtime.readSessionState({ sessionId, expectedRevision: 1 });
		assert(native.ok);
		...
		const definition = OPENFAIRYGUI_BACKEND_TOOL_DEFINITIONS.find(
			(entry) => entry.backendMethod === 'readSessionState',
		)!;
		const invalid = structuredClone(backend);
		Object.assign(invalid.data.project.packages[0].resources.find((entry) => entry.kind === 'image')!, {
			sourceBytes: [1, 2],
		});
		...
		const bytes = await client.callTool({
			name: 'openfairygui_backend_read_resource_bytes',
			arguments: { sessionId, expectedRevision: 1, selector },
		});
		t.false(bytes.isError);
		const source = runtime.readResourceBytes({ sessionId, expectedRevision: 1, selector });
		assert(source.ok);
		...
		for (const method of ['read_session_state', 'read_resource_bytes']) {
			const stale = await client.callTool({
				name: `openfairygui_backend_${method}`,
				arguments: {
					sessionId,
					expectedRevision: 0,
					...(method === 'read_resource_bytes' ? { selector } : {}),
				},
			});
			t.true(stale.isError);
			t.is(
				(stale.structuredContent as { backendResult: { error: { code: string } } }).backendResult.error.code,
				'stale_read',
			);
		}
		t.true(
		...
		const after = runtime.readSessionState({ sessionId });
		assert(after.ok);
		t.deepEqual(after.data, native.data);
	} finally {
...
test('MCP bounds the full session-read response independently of Backend limits', async (t) => {
	...
	try {
		...
		const error = (result.structuredContent as { backendResult: { error: { code: string; maxBytes: number } } })
			.backendResult.error;
		t.is(error.code, 'mcp_response_budget_exceeded');
		t.is(error.maxBytes, definition.maxResponseBytes);
...
test('MCP reads complete project and package settings before editing only the requested fields', async (t) => {
	...
	try {
		...
		for (const target of [
			...
			{ kind: 'package', selector: {} },
		]) {
			t.true(
				(await client.callTool({ name: 'openfairygui_backend_query_entity', arguments: { sessionId, target } }))
					.isError,
			);
		}
		const projectQuery = await call<BackendEntitySnapshot>('query_entity', {
			sessionId,
			target: { kind: 'project' },
		});
		const packageQuery = await call<BackendEntitySnapshot>('query_entity', {
			sessionId,
		...
		t.deepEqual(packageQuery.entity.properties.settings, {
			compressPNG: expected.packages[0].compressPNG,
			jpegQuality: expected.packages[0].jpegQuality,
			publish: expected.packages[0].publish,
		});
		...
		expected.settings.common.fontSize = 24;
		expected.packages[0].jpegQuality = 90;
		const transaction = {
			sessionId,
			expectedRevision: projectQuery.revision,
			operations: [
				{ kind: 'updateProjectSettings', settings: projectQuery.entity.properties.settings },
				{
					...
					settings: packageQuery.entity.properties.settings,
				},
			],
		};
		...
		const projectAgain = await call<BackendEntitySnapshot>('query_entity', {
			sessionId,
			target: projectQuery.target,
		});
		const packageAgain = await call<BackendEntitySnapshot>('query_entity', {
			sessionId,
			target: packageQuery.target,
		});
		assert(projectAgain.entity.kind === 'project' && packageAgain.entity.kind === 'package');
		t.is(projectAgain.revision, applied.revision);
		t.is(packageAgain.revision, applied.revision);
...
test('MCP queries, previews, applies and saves with generated schemas and revision checks', async (t) => {
	...
	try {
		...
		t.true(tool?.annotations?.readOnlyHint);
		t.true(
			tools.tools.find((entry) => entry.name === 'openfairygui_backend_preflight_transaction')?.annotations
				?.readOnlyHint,
		);
		const result = await client.callTool({
		...
		const invalid = await client.callTool({
			...
			arguments: {
				sessionId: opened.data.sessionId,
				target: { kind: 'resource', selector: { packageId: 'pkg001' } },
			},
		});
			...
			operations: [
				{
					kind: 'setDisplayNodeProps',
					selector: { packageId: 'pkg001', componentResourceId: 'cmp001', displayNodeId: 'n1' },
					props: { text: 'MCP safe edit' },
				},
			],
		};
		const events = runtime.getEvents({ sessionId: opened.data.sessionId });
		const binaryPreview = await client.callTool({
		...
		const failure = (rejected.structuredContent as { backendResult: BackendResult<BackendTransactionPreview> })
			.backendResult;
		...
		const preview = (previewed.structuredContent as { backendResult: BackendResult<BackendTransactionPreview> })
			.backendResult;
		assert(preview.ok);
		...
		t.deepEqual(
			preview.data.impact.entities.find(
				(entry) => entry.target.kind === 'displayNode' && entry.target.selector.displayNodeId === 'n1',
			),
		...
		const unchanged = await client.callTool({
			name: 'openfairygui_backend_query_entity',
			arguments: { sessionId: opened.data.sessionId, target: snapshot.target },
		});
		const unchangedResult = (unchanged.structuredContent as { backendResult: BackendResult<BackendEntitySnapshot> })
			.backendResult;
		assert(unchangedResult.ok);
		t.deepEqual(unchangedResult.data, snapshot);
		const applied = await client.callTool({
			name: 'openfairygui_backend_apply_transaction',
			arguments: transaction,
		});
		t.false(applied.isError);
		const changed = (applied.structuredContent as { backendResult: BackendResult<BackendSessionSnapshot> })
			.backendResult;
		assert(changed.ok);
		...
		const saved = await client.callTool({
			...
			arguments: { sessionId: opened.data.sessionId, expectedRevision: changed.data.revision },
		});
...
test('MCP complex queries validate formal selectors and round-trip full snapshots without losing untouched data', async (t) => {
	...
	composeController(doc, component, {
		name: 'state',
		pages: [
			{ id: '0', name: 'Idle', remark: 'Preserve me' },
			{ id: '1', name: 'Active' },
		],
		...
		for (const kind of ['controller', 'transition'] as const) {
			...
			const target = {
				...
				selector: {
					...
					[key]: kind === 'controller' ? 'state' : 'intro',
				},
			};
			const result = await client.callTool({
				name: 'openfairygui_backend_query_entity',
			...
			t.false(result.isError);
			const queried = (result.structuredContent as { backendResult: BackendResult<BackendEntitySnapshot> })
				.backendResult;
			assert(queried.ok);
			...
			if (queried.data.entity.kind === 'controller') {
				...
				operation = {
					...
					controller: queried.data.entity.properties,
				};
			} else {
				...
				operation = {
					...
					selector: target.selector,
					transition: queried.data.entity.properties,
				};
			}
			const before = runtime.getSession({ sessionId });
			assert(before.ok);
			for (const selector of [
				{ packageId: 'pkg001', componentResourceId: 'cmp001' },
				{ ...target.selector, [key]: '' },
				{ ...target.selector, [key]: 'x'.repeat(257) },
				{ ...target.selector, inventedId: 'x' },
			]) {
				t.true(
					(
						await client.callTool({
							name: 'openfairygui_backend_query_entity',
							arguments: { sessionId, target: { kind, selector } },
						})
					).isError,
				);
			}
			const absent = await client.callTool({
				name: 'openfairygui_backend_query_entity',
				arguments: { sessionId, target: { kind, selector: { ...target.selector, [key]: 'absent' } } },
			});
			const failure = (absent.structuredContent as { backendResult: BackendResult<BackendEntitySnapshot> })
				.backendResult;
			assert(!failure.ok && failure.error.code === 'entity_query_failed');
			t.is(failure.error.reason, 'not_found');
			...
			t.false(
				(await client.callTool({ name: 'openfairygui_backend_preflight_transaction', arguments: transaction }))
					.isError,
			);
			t.false(
				(await client.callTool({ name: 'openfairygui_backend_apply_transaction', arguments: transaction }))
					.isError,
			);
			t.true(
				(await client.callTool({ name: 'openfairygui_backend_apply_transaction', arguments: transaction }))
