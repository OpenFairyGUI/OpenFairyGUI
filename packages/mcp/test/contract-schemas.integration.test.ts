...
test('operation discovery returns precise, isolated schemas including nested properties and byte arrays', (t) => {
	...
	const apply = OPENFAIRYGUI_BACKEND_TOOL_DEFINITIONS.find(
		(entry) => entry.backendMethod === 'applyTransaction',
	)!.inputSchema;
	const input = {
		sessionId: 's',
		expectedRevision: 0,
	...
	const open = OPENFAIRYGUI_BACKEND_TOOL_DEFINITIONS.find(
		(entry) => entry.backendMethod === 'openProjectSession',
	)!.inputSchema;
...
test('each generated output schema validates its own data and error types', (t) => {
	...
	const getCapabilities = OPENFAIRYGUI_BACKEND_TOOL_DEFINITIONS.find(
		(entry) => entry.backendMethod === 'getCapabilities',
	)!;
	...
	t.false(
		getSession.outputSchema.safeParse({
			backendResult: { ok: false, meta: {}, error: { code: 'made_up', message: 'bad' } },
		}).success,
	);
});
...
test('wire schemas use uniform items for fixed numeric tuples without weakening their validation', async (t) => {
	...
	try {
					...
					if ('$ref' in value) {
						assert(
							typeof value.$ref === 'string' &&
								(value.$ref === '#' || value.$ref.startsWith('#/definitions/')),
							'Only self-contained references are allowed',
						);
						assert(
							value.$ref
								.split('/')
		...
		for (const method of ['apply_transaction', 'preflight_transaction']) {
			...
			const inline = z.toJSONSchema(definition.inputSchema, {
				target: 'draft-07',
				io: 'input',
				reused: 'inline',
			});
			t.true(
				Buffer.byteLength(JSON.stringify(wire.inputSchema)) < Buffer.byteLength(JSON.stringify(inline)) / 4,
				'Discovery must not re-expand shared transaction schemas',
			);
			...
			for (const { kind } of getOpenFairyGuiOperationCatalog().operations)
				t.true(kinds.has(kind), `Missing operation: ${kind}`);
			const rename = {
				sessionId: 's',
				expectedRevision: 0,
				operations: [
					{ kind: 'renameResource', selector: { packageId: 'p', resourceId: 'r' }, newName: 'After' },
				],
			};
			...
			for (const invalid of [
				...
				{ ...rename, operations: [] },
				{ ...rename, operations: [{ kind: 'invented' }] },
				{
					...rename,
					operations: [
						{
				...
				t.false(schema.safeParse(invalid).success);
			for (const operation of [
				...
				for (const value of [null, [0, 1, 2, 3]])
					t.true(parse(value), `${method}: ${operation.kind} ${JSON.stringify(value)}`);
				for (const value of [[], [1, 2, 3], [1, 2, 3, 4, 5], [1, 2, 3, '4']])
					t.false(parse(value), `${method}: ${operation.kind} ${JSON.stringify(value)}`);
			}
		}
	} finally {
		await client.close();
...
test('direct calls reject structural and budget violations before reaching Backend', async (t) => {
	const runtime = new BackendRuntime();
	let calls = 0;
	runtime.applyTransaction = async () => {
		calls++;
		throw new Error('Must not be called');
	};
	await t.throwsAsync(
		callOpenFairyGuiBackendTool(runtime, 'openfairygui_backend_apply_transaction', {
			sessionId: 's',
			expectedRevision: 0,
			operations: [{ kind: 'invented' }],
		}),
	);
	let nested: unknown = null;
	for (let i = 0; i < 35; i++) nested = [nested];
	await t.throwsAsync(callOpenFairyGuiBackendTool(runtime, 'openfairygui_backend_apply_transaction', { nested }), {
...
test('MCP discovery and four representative operations preserve semantics through save and reread', async (t) => {
	...
	const server = createOpenFairyGuiMcpServer({
		runtime: createNodeBackendRuntime({ allowedProjectRoots: [fixture.rootDir] }),
	});
		...
		const structured = result.structuredContent as
			| {
					backendResult?: {
						ok: boolean;
						error?: unknown;
						data: { sessionId: string; revision: number; dirty: boolean };
					};
			  }
			| undefined;
	...
	try {
		...
		component.id = 'cmp002';
		component.name = 'Copy';
		const renamed = await call('apply_transaction', {
			sessionId,
		...
		const applied = await call('apply_transaction', {
			sessionId,
			...
			operations: [
				{
					...
					props: { text: 'Contract-backed edit' },
				},
		...
		const replaced = await call('apply_transaction', {
			sessionId,
			expectedRevision: applied.revision,
			operations: [
				{
					kind: 'replaceResourceBytes',
					selector: { packageId: 'pkg001', resourceId: 'data001' },
					sourceBytes: [0, 255, 42],
				},
			],
		});
		...
		t.deepEqual(
			resources.find((entry) => entry.id === 'cmp002'),
			component,
		);
		t.deepEqual(
			[...(await fs.readFile(path.join(fixture.rootDir, 'assets', project.packages[0].name, 'data.bin')))],
			[0, 255, 42],
		);
	} finally {
