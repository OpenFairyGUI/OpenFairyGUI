...
test('all value gears retain an absent default through normalization, transactions and binary encoding', (t) => {
	...
	const cases = [
		[GearType.Size, '100,40,2,2'],
		[GearType.Look, '0.8,30,false,true'],
		[GearType.Color, '#654321'],
		[GearType.Animation, '8,p'],
		[GearType.FontSize, '40'],
	] as const;
	for (const [type, value] of cases) {
		const child = doc
			.createGTextField(`child${type}`)
			.setId(`n${type}`)
			.setSize(80, 30)
			.setScale(0.5, 0.75)
			.setAlpha(0.4)
	...
	for (const [index, [type]] of cases.entries()) {
		...
		const sourceBytes = new WriteBuffer(),
			resultBytes = new WriteBuffer();
