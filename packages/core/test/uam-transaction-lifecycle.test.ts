...
test('addResource restores exact resource order with optional stable indexes', (t) => {
	...
	const orderedA = createMisc('ordered-a', 1);
	const orderedB = createMisc('ordered-b', 2);
	pkg.resources.splice(1, 0, orderedA);
	pkg.resources.push(orderedB);
	...
	t.is(validateTransactionSupport(invalidSource, invalidOperations)
		.filter((issue) => issue.code === 'invalid_resource_index').length, 2);
	...
	t.is(documentApplied.packages[0]!.resources[1]?.id, snapshots[0]!.id);
});
