...
test('all project settings sidecars survive detached UAM round-trip and independent write', async (t) => {
	...
	if (
		!Array.isArray(liftedGroups) ||
		typeof liftedGroups[0] !== 'object' ||
		liftedGroups[0] === null ||
		Array.isArray(liftedGroups[0])
	) {
...
test('ProjectWriter only accepts structured, package-scoped stale source references', async (t) => {
	...
	const error = await t.throwsAsync(
		new ProjectWriter(fs).write(createWritableDocument(), 'Project.fairy', {
...
test('ProjectWriter shares branch targets and descriptor ordering while preserving source bytes', async (t) => {
	...
	pkg.addResource(
		doc
			.createMiscResource('data')
			.setId('x')
			.setFile('source.bin')
			.setBranch('resourceOnly')
	...
	t.deepEqual(
		[...output.files.keys()].sort(),
		[
			'Project.fairy',
			'settings/Common.json',
			'assets/Demo/package.xml',
			'assets/Demo/Main.xml',
			'assets/Demo/nested/raw.png',
			'assets/Demo/nested/existing.png',
			'assets_empty/Demo/package_branch.xml',
			'assets_mobile/Demo/package_branch.xml',
			'assets_mobile/Demo/localized/Main.xml',
			'assets_resourceOnly/Demo/package_branch.xml',
			'assets_resourceOnly/Demo/source.bin',
			'assets_folderOnly/Demo/package_branch.xml',
		].sort(),
	);
	...
	t.deepEqual(
		[...xml.matchAll(/<(?:component|image) id="([^"]+)"/g)].map((match) => match[1]),
		['b', 'z', 'cmpMain'],
	);
...
test('ProjectWriter preflights branch target collisions and leaves stale sources after a write failure', async (t) => {
	...
	await t.throwsAsync(new ProjectWriter(output).write(doc, 'Project.fairy'), {
		message: /conflicts with package descriptor/,
	});
	...
	output.writeFileRaw = async () => {
		throw new Error('output failed');
	};
	await t.throwsAsync(
		new ProjectWriter(output).write(doc, 'Project.fairy', {
