...
function normalizeOrderedChildren(
	...
	for (const [index, entry] of elements.entries()) {
		...
		for (const [tagName, value] of Object.entries(entry)) {
			...
			const rawNestedEntries = Array.isArray(rawEntry[tagName]) ? (rawEntry[tagName] as OrderedXmlEntry[]) : [];
			const normalizedChildren = normalizeOrderedChildren(nestedEntries, rawNestedEntries);
			const normalizedAttrs =
				tagName === 'property' && typeof rawAttrs.value === 'string'
					? { ...attrs, value: rawAttrs.value }
					: attrs;
			const normalizedValue =
				Object.keys(normalizedChildren).length > 0
...
function getOrderedDisplayListItems(xmlContent: string): Array<{ tagName: string; attrs: DisplayObjectXmlNode }> {
	...
	return displayListChildren.flatMap((entry, index) => {
		...
		const rawNestedEntries = Array.isArray(rawEntry[rawTagName]) ? (rawEntry[rawTagName] as OrderedXmlEntry[]) : [];
...
function readRawDisplayListAttrs(xmlContent: string, tagName: string, id: unknown): Record<string, unknown> {
...
function parseButtonDownEffect(value: unknown): number {
	return (
		({ none: 0, dark: 1, scale: 2 } as Record<string, number>)[
			String(value ?? '')
				.trim()
				.toLowerCase()
		] ?? 0
...
export function readComponentXml(ctx: ReaderContext, comp: Component, xmlContent: string): void {
	...
	const bgColorEnabled = readXmlAttr<string | boolean>(
		compNode,
		PROJECT_XML_PROTOCOL.componentRoot.attrs.bgColorEnabled,
	);
	...
	const designImageForTest = readXmlAttr<string | boolean>(
		compNode,
		PROJECT_XML_PROTOCOL.componentRoot.attrs.designImageForTest,
	);
	if (designImageForTest !== undefined) comp.setDesignImageForTest?.(parseBool(designImageForTest));
	const designImageAlpha = readXmlAttr<string | number>(
		compNode,
		PROJECT_XML_PROTOCOL.componentRoot.attrs.designImageAlpha,
	);
	if (designImageAlpha !== undefined) comp.setDesignImageAlpha?.(parseInt2(designImageAlpha));
	const designImageLayer = readXmlAttr<string | number>(
		compNode,
		PROJECT_XML_PROTOCOL.componentRoot.attrs.designImageLayer,
	);
	if (designImageLayer !== undefined) comp.setDesignImageLayer?.(parseInt2(designImageLayer));
	const designImageOffsetX = readXmlAttr<string | number>(
		compNode,
		PROJECT_XML_PROTOCOL.componentRoot.attrs.designImageOffsetX,
	);
	if (designImageOffsetX !== undefined) comp.setDesignImageOffsetX?.(parseInt2(designImageOffsetX));
	const designImageOffsetY = readXmlAttr<string | number>(
		compNode,
		PROJECT_XML_PROTOCOL.componentRoot.attrs.designImageOffsetY,
	);
	...
	if (overflow === 'scroll') {
		...
		const scrollBarFlags = readXmlAttr<string | number>(
			compNode,
			PROJECT_XML_PROTOCOL.componentRoot.attrs.scrollBarFlags,
		);
		...
		if (extType) {
			...
			if (extElement) {
				...
				if (extAttrs) {
					switch (extention) {
						case 'Button':
							if (readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.Button.attrs.mode) !== undefined)
								comp.setButtonMode?.(
									parseButtonMode(readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.Button.attrs.mode)!),
								);
							if (readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.Button.attrs.sound) !== undefined)
								comp.setSound?.(
									String(readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.Button.attrs.sound)),
								);
							if (
								readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.Button.attrs.soundVolumeScale) !==
								undefined
							)
								comp.setSoundVolumeScale?.(
									parseFloat2(
										readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.Button.attrs.soundVolumeScale),
										100,
									) / 100,
								);
							if (readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.Button.attrs.downEffect) !== undefined)
								comp.setDownEffect?.(
									parseButtonDownEffect(
										readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.Button.attrs.downEffect),
									),
								);
							if (
								readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.Button.attrs.downEffectValue) !== undefined
							)
								comp.setDownEffectValue?.(
									parseFloat2(
										readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.Button.attrs.downEffectValue),
										0.8,
									),
								);
							break;
						case 'ComboBox':
							if (readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.ComboBox.attrs.dropdown) !== undefined)
								comp.setDropdown?.(
									String(readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.ComboBox.attrs.dropdown)),
								);
							if (
								readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.ComboBox.attrs.selectionController) !==
								undefined
							)
								comp.setSelectionController?.(
									String(
										readXmlAttr(
											extAttrs,
											EXTENSION_PROTOCOL_MAP.ComboBox.attrs.selectionController,
										),
									),
								);
							if (
								readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.ComboBox.attrs.autoClearItems) !==
								undefined
							)
								comp.setAutoClearItems?.(
									parseBool(
										readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.ComboBox.attrs.autoClearItems),
									),
								);
							break;
						case 'Label':
							if (readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.Label.attrs.prompt) !== undefined)
								comp.setPromptText?.(
									String(readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.Label.attrs.prompt)),
								);
							break;
						case 'ProgressBar':
							if (readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.ProgressBar.attrs.titleType) !== undefined)
								comp.setTitleType?.(
									parseTitleType(
										readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.ProgressBar.attrs.titleType)!,
									),
								);
							if (readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.ProgressBar.attrs.reverse) !== undefined)
								comp.setReverse?.(
									parseBool(readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.ProgressBar.attrs.reverse)),
								);
							break;
						case 'Slider':
							if (readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.Slider.attrs.titleType) !== undefined)
								comp.setTitleType?.(
									parseTitleType(
										readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.Slider.attrs.titleType)!,
									),
								);
							if (readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.Slider.attrs.reverse) !== undefined)
								comp.setReverse?.(
									parseBool(readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.Slider.attrs.reverse)),
								);
							if (readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.Slider.attrs.wholeNumbers) !== undefined)
								comp.setWholeNumbers?.(
									parseBool(readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.Slider.attrs.wholeNumbers)),
								);
							if (readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.Slider.attrs.changeOnClick) !== undefined)
								comp.setChangeOnClick?.(
									parseBool(readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.Slider.attrs.changeOnClick)),
								);
							break;
						case 'ScrollBar':
							if (
								readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.ScrollBar.attrs.fixedGripSize) !==
								undefined
							)
								comp.setFixedGripSize?.(
									parseBool(
										readXmlAttr(extAttrs, EXTENSION_PROTOCOL_MAP.ScrollBar.attrs.fixedGripSize),
									),
								);
	...
	comp.setCustomProperties(
		customProperties.flatMap((value) => {
	...
	for (const ctrlDef of controllers) {
		...
		ctrl.setSelectedIndex(parseInt2(selected))
			...
			.setAutoRadioGroupDepth(
				parseBool(readXmlAttr(ctrlDef, PROJECT_XML_PROTOCOL.controller.attrs.autoRadioGroupDepth)),
			)
		...
		for (let actionIndex = 0; actionIndex < actions.length; actionIndex += 1) {
			...
			const actionType = parseControllerActionType(
				readXmlAttr<string>(actionDef, PROJECT_XML_PROTOCOL.controllerAction.attrs.type),
			);
			...
			switch (actionType) {
				case ControllerActionType.PlayTransition: {
					const transitionName = readXmlAttr<string>(
						actionDef,
						PROJECT_XML_PROTOCOL.controllerAction.attrs.transition,
					);
					const repeat = readXmlAttr<string | number>(
						actionDef,
						PROJECT_XML_PROTOCOL.controllerAction.attrs.repeat,
					);
					const delay = readXmlAttr<string | number>(
						actionDef,
						PROJECT_XML_PROTOCOL.controllerAction.attrs.delay,
					);
					const stopOnExit = readXmlAttr<string | boolean>(
						actionDef,
						PROJECT_XML_PROTOCOL.controllerAction.attrs.stopOnExit,
					);
					action
				...
				case ControllerActionType.ChangePage: {
					const objectId = readXmlAttr<string>(
						actionDef,
						PROJECT_XML_PROTOCOL.controllerAction.attrs.objectId,
					);
					const controllerName = readXmlAttr<string>(
						actionDef,
						PROJECT_XML_PROTOCOL.controllerAction.attrs.controller,
					);
					const targetPage = readXmlAttr<string>(
						actionDef,
						PROJECT_XML_PROTOCOL.controllerAction.attrs.targetPage,
					);
					action
	...
	for (const transDef of transitions) {
		...
		const autoPlayTimes = readXmlAttr<string | number>(
			transDef,
			PROJECT_XML_PROTOCOL.transition.attrs.autoPlayTimes,
		);
		const autoPlayDelay = readXmlAttr<string | number>(
			transDef,
			PROJECT_XML_PROTOCOL.transition.attrs.autoPlayDelay,
		);
		...
		for (const itemDef of items) {
			...
			const duration = readXmlAttr<string | number>(
				parsedItem,
				PROJECT_XML_PROTOCOL.transitionItem.attrs.duration,
			);
			...
			const customEaseValue = readXmlAttr<string>(
				parsedItem,
				PROJECT_XML_PROTOCOL.transitionItem.attrs.customEase,
			);
			...
			const typeStr = (
				readXmlAttr<string>(parsedItem, PROJECT_XML_PROTOCOL.transitionItem.attrs.type) || ''
			).toUpperCase();
