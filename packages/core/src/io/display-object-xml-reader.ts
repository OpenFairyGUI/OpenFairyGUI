...
import { parseBool, parseFloat2, parseInt2, parseSizeString, parseXYString } from '../utils/xml-utils.js';
...
function readCommonDisplayState(
	...
	const alpha = specs.alpha ? readXmlAttr<string | number>(source, specs.alpha) : undefined;
	if (alpha !== undefined) object.setAlpha?.(parseFloat2(alpha, 1));

	const rotation = specs.rotation ? readXmlAttr<string | number>(source, specs.rotation) : undefined;
	if (rotation !== undefined) object.setRotation?.(parseFloat2(rotation));

	const visible = specs.visible ? readXmlAttr<string | boolean>(source, specs.visible) : undefined;
	if (visible !== undefined) object.setVisible?.(parseBool(visible));

	const touchable = specs.touchable ? readXmlAttr<string | boolean>(source, specs.touchable) : undefined;
	if (touchable !== undefined) object.setTouchable?.(parseBool(touchable));

	const grayed = specs.grayed ? readXmlAttr<string | boolean>(source, specs.grayed) : undefined;
...
export function createDisplayObject(
	...
	switch (tagName) {
		case 'image': {
			...
			const imageFillClockwise = readXmlAttr<string | boolean>(
				attrs,
				PROJECT_XML_PROTOCOL.image.attrs.fillClockwise,
			);
			const imageFillAmount = readXmlAttr<string | number>(attrs, PROJECT_XML_PROTOCOL.image.attrs.fillAmount);
			if (
				imageFillMethod ||
				imageFillOrigin !== undefined ||
				imageFillClockwise !== undefined ||
				imageFillAmount !== undefined
			) {
		...
		case 'group': {
			...
			const excludeInvisibles = readXmlAttr<string | boolean>(
				attrs,
				PROJECT_XML_PROTOCOL.group.attrs.excludeInvisibles,
			);
			if (excludeInvisibles !== undefined) g.setExcludeInvisibles?.(parseBool(excludeInvisibles));
			const autoSizeDisabled = readXmlAttr<string | boolean>(
				attrs,
				PROJECT_XML_PROTOCOL.group.attrs.autoSizeDisabled,
			);
		...
		case 'loader': {
			...
			const clearOnPublish = readXmlAttr<string | boolean>(
				attrs,
				PROJECT_XML_PROTOCOL.loader.attrs.clearOnPublish,
			);
			...
			if (fillMethod) {
				...
				const fillClockwise = readXmlAttr<string | boolean>(
					attrs,
					PROJECT_XML_PROTOCOL.loader.attrs.fillClockwise,
				);
		...
		case 'loader3d': {
			...
			const loader3dShrinkOnly = readXmlAttr<string | boolean>(
				attrs,
				PROJECT_XML_PROTOCOL.loader3D.attrs.shrinkOnly,
			);
			...
			const clearOnPublish = readXmlAttr<string | boolean>(
				attrs,
				PROJECT_XML_PROTOCOL.loader3D.attrs.clearOnPublish,
			);
			if (clearOnPublish !== undefined) g.setClearOnPublish(parseBool(clearOnPublish));
			obj = g;
			break;
		...
		case 'component': {
			...
			const controllerOverrides = readXmlAttr<string>(
				attrs,
				PROJECT_XML_PROTOCOL.componentInstance.attrs.controllerOverrides,
			);
			if (controllerOverrides) g.setControllerOverrides?.(controllerOverrides);
			const pageController = readXmlAttr<string>(
				attrs,
				PROJECT_XML_PROTOCOL.componentInstance.attrs.pageController,
			);
