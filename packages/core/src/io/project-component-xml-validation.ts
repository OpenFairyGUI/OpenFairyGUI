...
export function validateComponentXmlValues(
	...
	const validateNumberTuple = (
		...
		addInvalidValue(
			...
			spec,
			path,
			`exactly ${partCount} finite number(s)`,
			(value) => {
	...
	const validateEnum = (
		...
		addInvalidValue(
			...
			(value) => values.includes(value.trim()),
			nodeId,
		);
	};
	...
	validateBooleanAttrs(
		componentNode,
		[rootAttrs.anchor, rootAttrs.opaque, rootAttrs.reversedMask, rootAttrs.bgColorEnabled],
		rootPath,
	);
	validateInt32Attrs(
		componentNode,
		[rootAttrs.scrollBarFlags, rootAttrs.designImageAlpha, rootAttrs.designImageLayer, rootAttrs.idnum],
		rootPath,
	);
	...
	for (const [tagName, definitions] of Object.entries(displayList)) {
		for (const definition of ensureArray(definitions)) {
			...
			if (displayAttrs) {
				...
				addInvalidValue(
					attrs,
					displayAttrs.rotation,
					nodePath,
					'a finite number',
					isProjectFiniteNumber,
					nodeId,
				);
				addInvalidValue(
					attrs,
					...
					(value) => isProjectFiniteNumber(value) && Number(value) >= 0 && Number(value) <= 1,
					nodeId,
				);
			}
			...
			if (displayAttrs && ['text', 'richtext', 'inputtext'].includes(normalizedTagName)) {
				...
				addInvalidValue(
					attrs,
					displayAttrs.fontSize,
					nodePath,
					'a positive signed 32-bit integer',
					(value) => isProjectInt32(value) && BigInt(value.trim()) > 0n,
					nodeId,
				);
				...
				validateEnum(
					...
					['none', 'both', 'height', 'shrink', 'ellipsis'],
					nodePath,
					nodeId,
				);
			}
			...
			if (displayAttrs && (normalizedTagName === 'list' || normalizedTagName === 'tree')) {
				...
				validateEnum(
					attrs,
					displayAttrs.selectionMode,
					['single', 'multiple', 'multipleSingleClick', 'none'],
					nodePath,
					nodeId,
				);
			...
			if (displayAttrs && normalizedTagName === 'group') {
				validateBooleanAttrs(
					attrs,
					[displayAttrs.advanced, displayAttrs.excludeInvisibles, displayAttrs.autoSizeDisabled],
					nodePath,
					nodeId,
				);
				validateInt32Attrs(
					attrs,
					[displayAttrs.lineGap, displayAttrs.columnGap, displayAttrs.mainGridIndex],
					nodePath,
					nodeId,
				);
			...
			if (displayAttrs && ['image', 'loader', 'loader3d'].includes(normalizedTagName)) {
				...
				validateInt32Attrs(
					attrs,
					[displayAttrs.frame, displayAttrs.fillOrigin, displayAttrs.fillAmount],
					nodePath,
					nodeId,
				);
				validateEnum(attrs, displayAttrs.align, ['left', 'center', 'right'], nodePath, nodeId);
				...
				validateEnum(
					attrs,
					displayAttrs.fillMethod,
					['none', 'hz', 'vt', 'radial90', 'radial180', 'radial360'],
					nodePath,
					nodeId,
				);
			}
