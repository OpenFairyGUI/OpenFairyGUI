...
> extends GObject<TProps, TType> {
	...
	public setPropertyOverrides(v: GComponentPropertyOverride[]): this {
		return this.setComponentProp(
			'propertyOverrides',
			v.map((property) => ({ ...property })),
		);
	}

	public getMargin(): [number, number, number, number] {
		return this.getComponentProp('margin');
	}
	public setMargin(v: [number, number, number, number]): this {
