...
function findInvalidJsonData(
	...
	if (Array.isArray(value)) {
		for (let index = 0; index < value.length; index += 1) {
			if (!(index in value))
				return { path: `${path}[${index}]`, message: 'Project settings arrays must not contain holes.' };
...
export function validateProjectSettingsPayload(
	...
	if (!isPlainRecord(settings)) {
		pushSupportIssue(issues, 'invalid_project_settings', path, 'Project settings must be a JSON object.', {
			operationKind,
		});
		return;
	}
	...
	if (publish !== undefined) {
		...
		if (!valid) {
			pushSupportIssue(
				...
				`${path}.publish`,
				'Publish settings contain an invalid typed field.',
				{ operationKind },
			);
		} else {
			for (const [key, numberFields, booleanFields, stringFields] of [
				[
					'atlasSetting',
					['maxSize', 'padding'],
					['paging', 'forceSquare', 'fast', 'allowRotation', 'trimImage', 'extractAlpha'],
					['sizeOption'],
				],
				[
					'codeGeneration',
					[],
					['allowGenCode', 'getMemberByName', 'ignoreNoname'],
					['classNamePrefix', 'codePath', 'codeType', 'memberNamePrefix', 'packageName'],
				],
			] as const) {
				...
				if (
					...
					pushSupportIssue(
						...
						'invalid_project_settings',
						`${path}.publish.${key}`,
						`Publish ${key} contains an invalid typed field.`,
						{ operationKind },
					);
				}
			}
	...
	if (common !== undefined) {
		...
		if (!valid) {
			pushSupportIssue(
				...
				'invalid_project_settings',
				`${path}.common`,
				'Common settings contain an invalid typed field.',
				{ operationKind },
			);
		} else if (
			common.scrollBars !== undefined &&
			...
			pushSupportIssue(
				issues,
				'invalid_project_settings',
				`${path}.common.scrollBars`,
				'Common scrollBars contain an invalid typed field.',
				{ operationKind },
			);
		}
	}
	...
	if (
		...
		pushSupportIssue(
			...
			'invalid_project_settings',
			`${path}.adaptation`,
			'Adaptation settings contain an invalid typed field.',
			{ operationKind },
		);
	}
	...
	if (settings.customProperties !== undefined && !isPlainRecord(settings.customProperties)) {
		pushSupportIssue(
			...
			'Custom properties settings must be a JSON object.',
			{ operationKind },
		);
	}
	...
	if (
		...
		(!isPlainRecord(i18n) ||
			...
			!i18n.langFiles.every(
				(entry) => isPlainRecord(entry) && typeof entry.name === 'string' && typeof entry.path === 'string',
			))
	) {
		pushSupportIssue(
			issues,
			'invalid_project_settings',
			`${path}.i18n`,
			'I18n settings require langFiles entries with string name and path.',
			{ operationKind },
		);
	}
}
...
export function validatePackageSettingsPayload(
	...
	if (!isPlainRecord(settings) || !hasExactKeys(settings, PACKAGE_SETTINGS_KEYS)) {
		pushInvalidPackageSettings(
			issues,
			path,
			'Package settings must be one complete typed snapshot.',
			operationKind,
		);
		return false;
	}
	if (settings.compressPNG !== null && typeof settings.compressPNG !== 'boolean') {
		pushInvalidPackageSettings(
			issues,
			`${path}.compressPNG`,
			'compressPNG must be boolean or null.',
			operationKind,
		);
	}
	if (settings.jpegQuality !== null && !isIntegerBetween(settings.jpegQuality, 1, 100)) {
		pushInvalidPackageSettings(
			issues,
			`${path}.jpegQuality`,
			'jpegQuality must be null or an integer from 1 to 100.',
			operationKind,
		);
	}
	if (settings.publish === null) {
		pushInvalidPackageSettings(
			issues,
			`${path}.publish`,
			'publish must be one complete typed snapshot.',
			operationKind,
		);
		return false;
	}
	...
	if (typeof publish.name === 'string' && publish.name && !isSafeBranchName(publish.name)) {
		pushInvalidPackageSettings(
			issues,
			`${path}.publish.name`,
			'Publish name must be empty or a safe output path segment.',
			operationKind,
		);
	}
	for (const key of ['path', 'branchPath', 'codePath'] as const) {
		if (typeof publish[key] === 'string' && !isSafePackageOutputPath(publish[key])) {
			pushInvalidPackageSettings(
				issues,
				`${path}.publish.${key}`,
				`${key} must be an empty or safe relative path.`,
				operationKind,
			);
		}
	}
	if (!isIntegerBetween(publish.packageCount, 0, 2_147_483_647)) {
		pushInvalidPackageSettings(
			issues,
			`${path}.publish.packageCount`,
			'packageCount must be a non-negative integer.',
			operationKind,
		);
	}
	for (const key of [
	...
	if (!isIntegerBetween(publish.maxAtlasSize, 1, 16_384)) {
		pushInvalidPackageSettings(
			issues,
			`${path}.publish.maxAtlasSize`,
			'maxAtlasSize must be an integer from 1 to 16384.',
			operationKind,
		);
	}
	if (publish.sizeOption !== 'pot' && publish.sizeOption !== 'npot' && publish.sizeOption !== 'mof') {
		pushInvalidPackageSettings(
			issues,
			`${path}.publish.sizeOption`,
			'sizeOption must be pot, npot, or mof.',
			operationKind,
		);
	}
	...
	if (!isIntegerBetween(publish.maxAtlasIndex, 0, 255)) {
		pushInvalidPackageSettings(
			issues,
			`${path}.publish.maxAtlasIndex`,
			'maxAtlasIndex must be an integer from 0 to 255.',
			operationKind,
		);
	}
		...
		for (const [atlasIndex, atlas] of publish.atlases.entries()) {
			...
			if (!isPlainRecord(atlas) || !hasExactKeys(atlas, PACKAGE_PUBLISH_ATLAS_KEYS)) {
				pushInvalidPackageSettings(
					issues,
					atlasPath,
					'Atlas entries must be complete typed snapshots.',
					operationKind,
				);
				continue;
			}
			if (
				typeof atlas.index !== 'number' ||
				!Number.isInteger(atlas.index) ||
				atlas.index < 0 ||
				atlas.index > maxAtlasIndex
			) {
				pushInvalidPackageSettings(
					issues,
					`${atlasPath}.index`,
					'Atlas index must be a non-negative integer no greater than maxAtlasIndex.',
					operationKind,
				);
			} else {
				if (indices.has(atlas.index)) {
					pushInvalidPackageSettings(
						issues,
						`${atlasPath}.index`,
						`Atlas index ${atlas.index} is duplicated.`,
						operationKind,
					);
				}
			...
			if (typeof atlas.name !== 'string' || (atlas.name && !isSafeBranchName(atlas.name))) {
				pushInvalidPackageSettings(
					issues,
					`${atlasPath}.name`,
					'Atlas name must be empty or a safe output path segment.',
					operationKind,
				);
			}
			if (typeof atlas.compression !== 'boolean') {
				pushInvalidPackageSettings(
					issues,
					`${atlasPath}.compression`,
					'Atlas compression must be boolean.',
					operationKind,
				);
				...
				pushInvalidPackageSettings(
					issues,
					atlasPath,
					'An atlas entry must define a name or enable compression.',
					operationKind,
				);
			}
		}
	...
	if (!Array.isArray(publish.excludedResourceIds)) {
		pushInvalidPackageSettings(
			issues,
			`${path}.publish.excludedResourceIds`,
			'excludedResourceIds must be an array.',
			operationKind,
		);
	} else {
		...
		for (const [resourceIndex, resourceId] of publish.excludedResourceIds.entries()) {
			...
			if (typeof resourceId !== 'string' || !/^[A-Za-z0-9_-]+$/.test(resourceId)) {
				pushInvalidPackageSettings(
					issues,
					resourcePath,
					'Excluded resource ids must be non-empty CSV-safe ids.',
					operationKind,
				);
				...
				pushInvalidPackageSettings(
					issues,
					resourcePath,
					`Excluded resource id "${resourceId}" is duplicated.`,
					operationKind,
				);
			}
			ids.add(resourceId);
