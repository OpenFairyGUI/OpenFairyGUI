...
export function isValidUamImageProperties(value: unknown): value is UamImageProperties {
	...
	return (
		...
		Number.isInteger(properties.flip) &&
		properties.flip >= 0 &&
		properties.flip <= 3 &&
		Number.isInteger(properties.fillMethod) &&
		properties.fillMethod >= 0 &&
		properties.fillMethod <= 5 &&
		Number.isInteger(properties.fillOrigin) &&
		properties.fillOrigin >= 0 &&
		properties.fillOrigin <= 3 &&
		typeof properties.fillClockwise === 'boolean' &&
		typeof properties.fillAmount === 'number' &&
		Number.isFinite(properties.fillAmount) &&
		(properties.fillMethod === 0
...
export function isValidUamMovieClipProperties(value: unknown): value is UamMovieClipProperties {
	...
	return (
		typeof properties.playing === 'boolean' &&
