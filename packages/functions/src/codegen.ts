...
import {
	...
	resolveCodeGenerationSettings,
	resolvePackageCodegenPlan,
	supportsCodeGenerationLane,
...
export async function publishCodeGeneration(doc: Document, options: PublishCodeGenerationOptions): Promise<void> {
	...
	for (const pkg of options.packages) {
		...
		if (!plan) {
			logger.warn(
				`publish: Code generation skipped for package "${pkg.getName()}" because no codePath was resolved.`,
			);
			continue;
		}
		...
		if (!supportsCodeGenerationLane(doc, settings.codeType)) {
			logger.warn(
				`publish: Code generation skipped for package "${pkg.getName()}" because project/codeType is not supported yet.`,
			);
			continue;
		}
