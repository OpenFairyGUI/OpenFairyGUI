...
function sourceName(resource: { name: string; fileName?: string; sourcePath?: string }): string {
...
export async function validateProjectWeb(project: UamProject): Promise<ProjectValidationReport> {
	...
	const hasHydratedImages = project.packages.some((pkg) =>
		pkg.resources.some((resource) => resource.kind === 'image' && resource.sourceBytes instanceof Uint8Array),
	);
		...
		for (const [resourceIndex, resource] of pkg.resources.entries()) {
			...
			try {
				...
				const capabilityUnavailable =
					error instanceof Error && /(?:required|unavailable)/iu.test(error.message);
				diagnostics.push({
	...
	const resolved = diagnostics.filter(
		(diagnostic) => diagnostic.code !== 'decode_capability_unavailable' || !decodedPaths.has(diagnostic.path),
	);
	return createProjectValidationReport(
		resolved,
		!resolved.some((diagnostic) => diagnostic.code === 'decode_capability_unavailable'),
	);
}
