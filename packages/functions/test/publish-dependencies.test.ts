...
test('publish: nullable and pipe Text/Icon states retain dependencies and icon atlas resources', async (t) => {
	...
	common.addResource(
		doc.createImageResource('remote.png').setId('remote').setWidth(8).setHeight(8).setExported(true),
	);
	for (const id of ['used', 'fallback', 'unused'])
		main.addResource(doc.createImageResource(`${id}.png`).setId(id).setPath('/').setWidth(8).setHeight(8));
	...
	text.addGear(
		doc
			.createGear()
			.setGearType(GearType.Text)
			.setController(controller)
			.setPages('0,1,2,3,4')
			.setPageValues(textValues)
			.setDefaultValue(''),
	);
	...
	icon.addGear(
		doc
			...
			.setController(controller)
			.setPages('0,1,2,3,4')
			.setPageValues(iconValues)
			.setDefaultValue('ui://main0001fallback'),
	);
	...
	t.deepEqual(
		main
			.listAtlases()
			.flatMap((atlas) => atlas.listSprites().map((sprite) => sprite.getItemId()))
			.sort(),
		['fallback', 'used'],
	);
	const decoded = (await new NodeIO().readBinary(binaryPath)).getRoot().getPackage('Main')!.getComponent('Host')!;
	t.deepEqual(decoded.listChildren()[0]!.listGears()[0]!.getPageValues(), {
		'1': '',
		'2': '-',
		'3': textValues['3'],
	});
	t.deepEqual(decoded.listChildren()[1]!.listGears()[0]!.getPageValues(), {
		'1': '',
		'2': '-',
		'3': iconValues['3'],
		'4': 'A|B',
...
test('publish: component instance button sound refs keep local sound items', async (t) => {
	...
	try {
		...
		t.true(
			parseItemIds(new Uint8Array(bytes.buffer, bytes.byteOffset, bytes.byteLength)).includes('snd001'),
			'published binary keeps referenced sound item',
		);
		t.deepEqual(
			[...(await fs.readFile(path.join(tmpDir, 'Main_snd001.wav')))],
			[...sourceSound],
			'published local sound is copied alongside the package binary',
		);
	} finally {
		await fs.rm(tmpDir, { recursive: true, force: true });
	}
...
test('publish: stage sounds keep local resources and cross-package dependencies', async (t) => {
	const sourceSound = await fs.readFile(
		getFixturePath('FairyGUI-unity', 'UIProject', 'assets', 'Basics', 'sound', 'click.wav'),
	);
	...
	try {
		for (const setter of ['setAddedToStageSound', 'setRemovedFromStageSound'] as const) {
			...
			audio.addResource(
				doc.createSoundResource('remote').setId('remote').setPath('/').setFile('click.wav').setExported(true),
			);
