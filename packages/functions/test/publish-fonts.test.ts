...
test('engine font references do not introduce a bitmap-font package dependency', async (t) => {
	...
	doc.getRoot().setProjectType(ProjectType.Unity);
	...
	const binary = result.files.find((file) => file.path.endsWith('.bytes'));
	assert(binary);
...
test('official Unity font pairs retain glyph textures and engine font names when republished', async (t) => {
	...
	for (const name of ['Basics', 'TextMeshPro']) {
		const published = await publishNode({
			document: doc,
			packages: [name],
			assetsPath: getFixturePath('FairyGUI-unity', 'UIProject', 'assets'),
		...
		const original = (
			await io.readBinary(
				getFixturePath('FairyGUI-unity', 'Assets', 'Examples', 'Resources', 'UI', `${name}_fui.bytes`),
			)
		)
			.getRoot()
			.getPackage(name)!;
		...
		if (name === 'Basics') {
			...
			const sprites = new Set(
				current.listAtlases().flatMap((atlas) => atlas.listSprites().map((sprite) => sprite.getItemId())),
			);
			...
			const fontValues = (pkg: typeof current) =>
				pkg
					.getComponent('Main')!
					.listChildren()
...
test('bitmap font dependency selection uses each branch source before merge or packing', async (t) => {
	...
	t.teardown(() => fs.rm(directory, { recursive: true, force: true }));
	const png = await sharp({ create: { width: 2, height: 2, channels: 4, background: '#123456' } })
		.png()
		.toBuffer();
	...
	for (const merged of [false, true]) {
		...
		doc.getRoot()
			...
			.setBranches(['en'])
			.setSettings({ publish: { branchProcessing: merged ? 1 : 0 } });
		const pkg = doc.createPackage('Main').setId('fontpkg1').setBranchNames(['en']);
		for (const branch of ['', 'en']) {
			pkg.addResource(
				doc
					.createFontResource('Digits')
					.setId(branch ? 'fonten01' : 'font0001')
					.setFileName('Digits.fnt')
					.setPath('/')
					.setBranch(branch)
					.setExported(true),
			);
			pkg.addResource(
				doc
					.createImageResource('glyph')
					.setId(branch ? 'glyphen1' : 'glyph001')
					.setFileName('glyph.png')
					.setPath('/')
					.setBranch(branch)
					.setWidth(2)
					.setHeight(2),
		...
		const result = await publishNode({
			...
			output: path.join(directory, `release-${merged}`),
			branch: 'en',
			encoder: sharp,
			plugins: [],
			codeGeneration: false,
		...
		t.deepEqual(
			fonts.map((font) => [font.getId(), font.listGlyphs()[0]!.getCharId(), font.listGlyphs()[0]!.getAdvance()]),
			merged
				? [['font0001', 66, 22]]
				: [
						['font0001', 65, 11],
		...
		for (const font of fonts) {
			...
			t.truthy(
				current.getResourceById(glyphId),
				`glyph dependency ${glyphId} must be in the same published package`,
			);
			t.true(
				current
					.listAtlases()
					.some((atlas) => atlas.listSprites().some((sprite) => sprite.getItemId() === glyphId)),
			);
		}
	}
});
