...
test('editing and reopening source XML preserves gear timing and component controller overrides', async (t) => {
	...
	await fs.writeFile(
		...
		'<packageDescription id="pkg001"><resources><component id="host" name="Host.xml" path="/" exported="true"/><component id="page" name="Page.xml" path="/"/></resources></packageDescription>',
	);
	await fs.writeFile(
		path.join(assets, 'Page.xml'),
		'<component size="100,100"><controller name="style" pages="0,Idle,1,Active" exported="true"/></component>',
	);
	// Source XML is independent of our writer, including saved timing while tweening is disabled.
	await fs.writeFile(
		path.join(assets, 'Host.xml'),
	...
	t.deepEqual(
		title
			.listGears()
			.sort((a, b) => a.getGearType() - b.getGearType())
			.map((gear) => [gear.getTween(), gear.getTweenDelay(), gear.getTweenDuration(), gear.getEaseType()]),
		[
			[false, 0.25, 0.6, 0],
			[true, 0.75, 0.3, 5],
		],
