...
test('backend responses carry unified diagnostics metadata', async (t) => {
	...
	try {
		...
		t.is(opened.meta.capabilitySchemaVersion, 15);
		...
		t.deepEqual(
			stale.meta.diagnostics.map(({ owner, docsUri, remediation, ...original }) => original),
			[
				{
					code: 'stale_write',
		...
		t.deepEqual(recovery.remediation?.read, {
			method: 'getProjectOutline',
			input: { sessionId: opened.data.sessionId },
		});
...
test('transaction failures expose stable editor-targeted diagnostics', async (t) => {
	...
	t.deepEqual(
		rejected.meta.diagnostics.map(({ owner, docsUri, remediation, ...original }) => original),
		[
			{
...
test('recovery keeps invalid selectors, source validation and unknown diagnostics distinct', async (t) => {
	...
	t.is(
		validation.meta.diagnostics.find((entry) => entry.code === 'decode_capability_unavailable')?.owner,
		'core.validation',
	);
	t.false(JSON.stringify(validation.data).includes('remediation'));
	const unknown = {
		code: 'host_extension_error',
		message: 'Host detail',
...
test('path denial recovery never grants wider access; catalog is detached and fails unknown codes', async (t) => {
	const fixture = await createTempBackendProject();
	try {
		const denied = await createBackendRuntime({ allowedProjectRoots: [fixture.rootDir + '-other'] }).openSession({
		...
		const saved = await runtime.saveSession({
			sessionId: opened.data.sessionId,
			expectedRevision: 0,
			targetPath: fixture.rootDir + '-other',
		});
		t.false(saved.ok);
	...
	t.is(new Set(catalog.map((guide) => guide.code)).size, 103);
	for (const guide of catalog) t.deepEqual(getBackendDiagnosticGuide(guide.code), guide);
	catalog.pop();
	t.is(getBackendDiagnosticCatalog().length, 103);
	t.deepEqual(getBackendDiagnosticGuide('invalid_uam').owners, ['core.transaction', 'core.validation']);
	t.is(
		createMeta('read', Date.now(), {
			diagnostics: [
				{ code: 'invalid_uam', severity: 'error', message: 'Invalid source', owner: 'core.validation' },
			],
		}).diagnostics[0]?.owner,
		'core.validation',
	);
