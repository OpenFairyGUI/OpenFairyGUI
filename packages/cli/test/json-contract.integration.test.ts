...
test('all CLI parser failures use the same JSON envelope, including root and nested options', (t) => {
	...
	for (const [args, command] of [
		...
		[['docs', '--json'], 'docs'],
		[['docs', 'unknown', '--json'], 'docs'],
		[['docs', 'cat', '--json'], 'docs cat'],
		[['--json', 'docs', 'schema'], 'docs schema'],
		[['docs', '--json', 'find'], 'docs find'],
		[['docs', 'diagnostic', '--json'], 'docs diagnostic'],
	] as const) {
		const run = spawnSync(process.execPath, ['--import', 'tsx/esm', entry, ...args], {
			encoding: 'utf8',
			timeout: 30_000,
		});
	...
	const help = spawnSync(process.execPath, ['--import', 'tsx/esm', entry, 'docs', '--help', '--json'], {
		encoding: 'utf8',
	});
	t.is(help.status, 0);
	t.true(help.stdout.startsWith('Usage:'));
});
