...
function cli(args: string[]) {
	return spawnSync(process.execPath, ['--import', 'tsx/esm', path.resolve('packages/cli/src/cli.ts'), ...args], {
		encoding: 'utf8',
		timeout: 30_000,
	});
}
	...
	try {
		for (const command of ['publish', 'restore']) {
			...
			for (const args of [[], [root, '--unknown'], [root, '--output']]) {
				...
				t.deepEqual(
					{ ...JSON.parse(result.stdout), error: undefined },
					{ schemaVersion: 1, command, success: false, error: undefined },
				);
			...
			t.is(JSON.parse(result.stdout).error.code, `${command}_failed`);
		}
		...
		document
			...
			.setId('pkgdemo1')
			.addResource(
				document.createComponent('MainView').setId('cmpdemo1').setPath('/').setSize(32, 24).setExported(true),
			);
		...
		await fs.writeFile(
			...
			JSON.stringify({ name: 'logger', main: 'index.cjs', openfairygui: true }),
		);
		await fs.writeFile(
			path.join(plugin, 'index.cjs'),
			'exports.onPublishStart = () => process.stdout.write("plugin output\\n");',
		);
		...
		t.true(report.success);
		t.is(report.result.files.length, 1);
		...
		t.is(refused.status, 1);
		t.is(JSON.parse(refused.stdout).error.code, 'restore_failed');
		...
		t.is(corrupt.status, 1);
		t.false(JSON.parse(corrupt.stdout).success);
