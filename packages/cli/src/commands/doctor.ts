...
async function inspectNativeImages(): Promise<DoctorReport['checks'][number]> {
	try {
		...
		return {
			id: 'native-images',
			status: 'incomplete',
			message: `Native image capability unavailable or failed; ask the host to check Sharp before publishing/restoring. ${error instanceof Error ? error.message : String(error)}`,
		};
	}
}
...
async function inspectDirectory(
	...
	const check: Extract<DoctorReport['checks'][number], { path: string }> = {
		...
		inspectedPath: path.resolve(requested),
		exists: null,
		message: '',
	};
	try {
		for (;;) {
			...
			if (!(await fs.stat(check.inspectedPath)).isDirectory())
				throw new Error('The inspected path is not a directory.');
			await fs.access(check.inspectedPath, constants.R_OK | constants.W_OK | constants.X_OK);
			check.status = 'ok';
			check.message = check.exists
				? 'Directory access flags passed; no files written.'
				: 'Target absent; only the nearest existing ancestor passed access flags. Nothing created.';
			break;
		}
	} catch (error) {
...
export function registerDoctorCommand(program: Command): void {
	program
		.command('doctor')
		.description(
			'Diagnose versions, native images and directory access; optionally validate a project; never write or repair',
		)
		.argument('[project-dir]', 'Optional project directory or .fairy file for Node validation')
		.option(
			'--output-dir <directory>',
			'Check output directory access (or its existing ancestor), without creating it',
			(value: string) => {
		...
		.action(async (projectDir: string | undefined, options: { json?: boolean; outputDir?: string }) => {
			...
			if (cliVersion !== version.packageVersion)
				errors.push({
					code: 'installed_version_mismatch',
					message: 'CLI and documentation versions differ; ask the host to reconcile the installation.',
				});
			const minimumMajor = /^>=(\d+)$/u.exec(version.nodeEngine)?.[1];
			if (!minimumMajor || Number(process.versions.node.split('.')[0]) < Number(minimumMajor))
				errors.push({
					code: 'unsupported_node_version',
					message: `Installed package requires Node ${version.nodeEngine}.`,
				});
			const capabilities = createNodeBackendRuntime().getCapabilities();
			const checks = await Promise.all([
				inspectNativeImages(),
				inspectDirectory('temp-directory', tmpdir()),
			...
			const status =
				...
				checks.some((check) => check.status === 'error')
					? 'error'
					: project?.status === 'incomplete' || checks.some((check) => check.status === 'incomplete')
						? 'incomplete'
						: 'ready';
			...
			if (options.json)
				printJson(
					...
					status === 'ready'
						? undefined
						: {
								code: status === 'error' ? 'doctor_failed' : 'doctor_incomplete',
								message: `Product diagnosis is ${status}; inspect result.checks, result.errors and result.project.`,
							},
				);
			else {
				console.log(
					`${status.toUpperCase()}: OpenFairyGUI ${cliVersion} (documentation ${version.packageVersion}, Node ${report.nodeVersion})`,
				);
				for (const error of errors) console.log(`${error.code}: ${error.message}`);
				for (const check of checks)
					console.log(
						`${check.status} ${check.id}${'path' in check ? ` ${check.path} (inspected ${check.inspectedPath})` : ''}: ${check.message}`,
					);
				if (project) {
