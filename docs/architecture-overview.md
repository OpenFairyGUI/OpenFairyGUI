...
- `@openfairygui/backend` 根入口当前提供 browser-safe async storage bridge；浏览器宿主把 OPFS、IndexedDB、ZIP 虚拟文件系统等实现适配为 `BackendFileSystem` 后，通过 `BackendRuntime({ fileSystem }).openSession()` 导入现有工程并执行 source-fidelity 检查。只有当 UAM 本身是事实来源时，才通过 `openProjectSession` 绑定 storage，并由 `materializeSession` 完成 workspace bootstrap / first write；`saveSession` 使用该 session 绑定的文件系统写回 dirty session。
