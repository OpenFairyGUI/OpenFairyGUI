...
预演的实体影响直接比较两份正式 UAM；文件影响复用既有 fidelity 检查的内存捕获文件系统，通过 Core ProjectWriter 序列化后比较，不新增封包/文件命名规则。返回完整有界的当前 revision 差异和保存前置状态，不能冒充真实磁盘写入计划或权限验证；详见[事务预演](./guide/contracts.md#预演一次事务)。
...
- `packages/core/src/uam/preflight/` 按不变量分工：settings（设置快照）、display（节点属性）、behaviors（controller / transition / gear 联动）、resources（资源与源字节）、resource-folders（空目录与 atlas）、lifecycle（按顺序投影结构及引用重写）、projected-state（最终 group / 资源引用）。support / values 提供原有支持范围、selector 与值校验；`transaction-preflight.ts` 保持入口、逐操作分发与阶段顺序，不新增注册表或另一套执行器。
