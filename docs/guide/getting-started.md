...
正式版 `0.6.0` 已提供 `readSessionState` / `readResourceBytes`。需要试用预发布版本时，将安装命令中的两个包换成 `@openfairygui/cli@next`、`@openfairygui/mcp@next`，仍保存精确版本。各包应使用同一版本与通道，操作依据来自当前安装语料。
...
主任务是把一个静态奖励面板改成由控制器驱动的三状态组件，一次编辑同时覆盖控制器、文字、按钮交互和显示条件。使用稳定版 `0.6.0` 即可完成本任务。
...
若宿主需要读取生成后尚未保存的完整模型，正式版 `0.6.0` 可用 `readSessionState`，再按返回 revision 调用 `readResourceBytes` 读取所需主资源。响应预算、读取诊断与 `stale_read` 的处理见[契约指南](./contracts.md)。
