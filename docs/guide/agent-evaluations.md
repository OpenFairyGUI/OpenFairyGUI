...
原始 stderr 显示 Codex 转换 apply/preflight 的 MCP schema 时报告 `invalid type: map, expected a string`，并跳过工具。宿主 `tools/list` 中确实包含这两个正式工具；不能据模型“host 未提供”的表述认定产品根本没有该方法。尚未定位到具体 schema 节点或客户端实现根因，本阶段未修复该解析问题。
