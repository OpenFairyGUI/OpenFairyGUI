...
See the [development guide](./guide/development.md) for setup, references and quality gates. Root/package AGENTS route work to existing owners; `agent/impact-map.json` drives test selection and the guidance table. Multi-version quality checks, documentation builds and isolated tarball consumers correspond to local `pnpm check:ci`. Consumers reuse the [runnable examples](./guide/examples.md); release checks the same five packed tarballs before publishing. Repository tools and examples are not shipped in product packages and do not change the runtime boundaries below.

## Summary

The fixed Backend MCP catalog uses existing Zod support to advertise repeated structures as self-contained draft-07 `definitions`/`$ref` in `tools/list`. Call validators, input budgets and Backend dispatch remain unchanged. Six installed-package agent evaluations check precise node edits, concurrent recovery and safe stopping with pending work intact in repository tooling, without new product repair or permission-bypass interfaces.
...
Preview entity impacts compare formal UAM snapshots directly. File impacts reuse the fidelity check's in-memory capture filesystem and Core ProjectWriter serialization, without new packaging or naming rules. The complete bounded current-revision diff and persistence prerequisites are not a disk write plan or permission verification. See [transaction previews](./guide/contracts.md#preview-a-transaction).

Contract authority remains in Core's UAM types/transaction entrypoint and Backend's public method signatures. The existing TypeScript compiler generates MCP structural schemas, operation catalogs and documentation tables. MCP declares transport metadata, host-field exclusions and input budgets; the original kernel still owns semantic validation. The snapshot and minimal version-bound corpus ship through the independent `@openfairygui/backend/docs` entrypoint. MCP builds Zod validators from that snapshot, and CLI/MCP share offline content without a reverse runtime dependency or Zod in Core. Product `ofgui doctor` reuses Node capabilities and `validateProjectNode` without opening sessions/locks. See [contracts](./guide/contracts.md) and [installed docs](./guide/installed-docs.md).
...
- `packages/core/src/uam/preflight/` separates invariants into settings snapshots, display properties, controller/transition/gear behaviors, resources and source bytes, resource-folders and atlas, ordered lifecycle projection/reference rewrites, and final projected-state group/resource references. Support/values retain shared scope, selector and value checks. `transaction-preflight.ts` keeps the entry points, per-operation dispatch and phase order, without a registry or another executor.
...
- `packages/mcp/src/*` provides the **thin backend P2 MCP adapter**. It maps `getCapabilities / openSession / openProjectSession / getSession / getProjectOutline / queryEntity / validateSession / preflightTransaction / applyTransaction / saveSession / materializeSession / closeSession / getEvents / getJob / listJobs / cancelJob / getCacheSnapshot / refreshCache`. The `openProjectSession` input schema fixes the top-level UAM project/package shape and collection budgets. `preflightTransaction` and `applyTransaction` use a `kind`-discriminated operation union and limits batch size, source bytes, recursive depth, node count, strings, and object keys. Each tool's precise input/output schema is generated from canonical Core/Backend types and preserves the successful/failed backend envelope. Its default Node runtime opens projects only below the process working directory; stdio accepts multiple canonical allowed roots through the platform-delimiter-separated `OPENFAIRYGUI_ALLOWED_PROJECT_ROOTS` environment variable.
