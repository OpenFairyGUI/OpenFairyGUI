...
Tool-only clients can call `openfairygui_docs_read({id:"index"})`, then use exact document IDs from the index. Default MCP instructions explain the safe editing sequence.
