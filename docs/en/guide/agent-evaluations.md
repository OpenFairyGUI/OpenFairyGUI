...
- Use the installed MCP server, schemas, Backend and Node filesystem. The editing host exposes ten session query/edit methods plus the read-only docs tool and official resource documentation; no virtual-project creation, materialize, publishing or arbitrary file commands.
...
`observations.discoveries` records compact UTF-8 JSON input/output schema bytes per tool and in total from every actual `tools/list`. These are not tokens, actual model context sizes or billing estimates. MCP reuses existing Zod local `definitions`/`$ref` support for repeated structures. The full product catalog provides the generated operation and method catalog; the evaluation host exposes only the restricted subset described above. Server structural validation, budgets and Backend safety boundaries remain unchanged. Clients may expand references internally; fewer wire bytes do not directly establish token savings.
...
`modelSuccessRate` is calculated for real model runners; it is always null for `reference`. Passing cases include read/edit objectives, two publish/recovery objectives and two correct safe stops, not exclusively successful edits. One small sample is neither a model ranking nor a stable success-rate guarantee. Missing tools, client parsing errors, timeouts and model mistakes require separate explanations from raw evidence, not just an aggregate score.

## CI gates

`pnpm test:repo` tests grader false positives, metrics, scope and CLI configuration. `pnpm pack:check` runs all **reference** tasks in the same tarball consumer to exercise real MCP, precise editing, conflict injection and safety refusals. These deterministic checks are part of `check:ci`.
...
Second client: `pnpm eval:agent --runner claude --claude <native-executable>`. Bare/restricted mode, disabled built-in tools and strict MCP configuration isolate the run; unexpected initialized tools or tool calls fail isolation. Flags follow the [official Claude Code reference](https://code.claude.com/docs/en/cli-reference). The compound-edit task requires edit/save, fresh query, rename/save. Versioned evidence belongs in agent/evals/baselines; reference results are not model scores. Real runs require authentication and a compatible native CLI; never report an unrun success rate.

Use `--report agent/evals/baselines/<version>-<runner>.json` to export versioned results, including failed runs. Reports retain installed versions, tarball/task hashes, dirty-checkout provenance and per-case checks; error stacks and full evidence remain outside the checkout.

Personal authentication settings are not inherited automatically. Bare mode does not read subscription OAuth credentials. Use client-supported API/provider environment authentication, or pass `--claude-settings <settings.json>` explicitly so Claude reads the file itself; the harness does not copy credentials. Supply only authentication/provider settings needed for the evaluation, without custom hooks, plugins or tools. This Claude-only option retains bare/restricted mode, disabled skills and built-in tools, and strict MCP configuration. Authentication failures, 429 responses and timeouts are environment failures: preserve the raw report, but do not interpret them as model capability scores. See [programmatic Claude authentication](https://code.claude.com/docs/en/headless#start-faster-with-bare-mode).

Failed tool calls retain primary and nested diagnostic codes in text; evaluations check both accurate blocker reporting and preservation of pending work.
