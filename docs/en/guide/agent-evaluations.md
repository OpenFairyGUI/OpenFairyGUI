...
`modelSuccessRate` is calculated only for `codex`; it is always null for `reference`. Eight passing cases mean six positive objectives and two correct safe stops, not eight successful edits. One small sample is neither a model ranking nor a stable success-rate guarantee. Missing tools, client parsing errors, timeouts and model mistakes require separate explanations from raw evidence, not just an aggregate score.

## CI gates

`pnpm test:repo` tests grader false positives, metrics, scope and CLI configuration. `pnpm pack:check` runs all eight **reference** tasks in the same tarball consumer to exercise real MCP, precise editing, conflict injection and safety refusals. These deterministic checks are part of `check:ci`.
