...
Read URLs from `.gitmodules`, fixed commits from gitlinks and `refs:status`, and source roles from `references.json`. Search hits are not protocol conclusions. Evidence order is official documentation → editor implementation including workers → paired source/published project → runtime consumption → supplementary material. New UI/runtime code cannot replace legacy exporter evidence. Optional local copies still require maintainer provenance/version verification. Stop conclusions needing missing decisive evidence; do not count skips as passes.
