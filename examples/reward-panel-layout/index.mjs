...
// #region example
export async function redesignRewardPanel(
	projectPath,
	...
	try {
		...
		const controller = data(
			await runtime.queryEntity({
				sessionId,
				target: { kind: 'controller', selector: { ...selector, controllerName: 'rewardState' } },
			}),
		);
		assert.equal(controller.revision, current.revision, 'Refresh after a concurrent edit');
		assert.deepEqual(
			controller.entity.properties.pages.map((page) => page.name),
			['Locked', 'Claimable', 'Claimed'],
			'Complete task A before this example',
		);
		assert(
			!components[0].component.transitions.some((transition) => transition.name === 'intro'),
			'An intro transition already exists; query and replan',
		);
		const layout = {
		...
		operations.push({
			kind: 'addTransition',
			selector: { ...selector, transitionName: 'intro' },
			transition: {
				name: 'intro',
				autoPlay: true,
				autoPlayTimes: 1,
				...
				items: [
					{
						...item,
						actionType: TransitionActionType.Alpha,
						startValue: [0],
						endValue: [1],
						label: 'fade-in',
					},
		...
		throw Object.assign(
			new Error('Reward panel redesign needs host recovery; the session remains open.', { cause }),
			{
...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
	...
	if (values.create)
		console.log(JSON.stringify({ projectPath, target: 'Main/RewardPanel', redesigned: false }, null, 2));
	else {
