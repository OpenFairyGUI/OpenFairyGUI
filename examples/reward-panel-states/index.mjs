...
export async function createRewardPanelProject(parent) {
	...
	panel.addChild(
		document
			.createGComponent('claimButton')
			.setId('claim-button')
			.setSrc('claimbtn')
			.setXY(80, 160)
			.setSize(200, 48)
			.setInstanceExtType('Button')
			.setInstanceTitle('领取奖励'),
	);
	panel.addChild(
		document
			.createGTextField('claimedMark')
			.setId('claimed-mark')
			.setXY(24, 228)
			.setSize(312, 28)
			.setText('✓ 奖励已领取')
...
// #region example
export async function editRewardPanel(
	projectPath,
	...
	try {
		...
		const components = packages[0].resources.filter(
			(resource) => resource.kind === 'component' && resource.name === 'RewardPanel',
		);
		...
		assert.equal(
			targets.claimButton.revision,
			targets.claimedMark.revision,
			'Refresh targets after a concurrent edit',
		);
		...
		const transaction = {
			sessionId,
			expectedRevision: targets.claimButton.revision,
			operations: [
				{
					kind: 'addController',
					selector: { ...selector, controllerName },
					controller: {
						name: controllerName,
						selectedIndex: 0,
				...
				{
					kind: 'addGear',
					selector: { ...targets.claimButton.target.selector, kind: 'text', controllerName },
					gear: {
						...common,
						kind: 'text',
						...
						states: pages.map((page, index) => ({
							pageId: page.id,
							value: { text: ['未达成', '领取奖励', '已领取'][index] },
						})),
					},
				},
				{
					kind: 'addGear',
					selector: { ...targets.claimButton.target.selector, kind: 'look', controllerName },
					gear: {
						...common,
						kind: 'look',
						...
						states: pages.map((page, index) => ({
							pageId: page.id,
							value: { alpha: 1, rotation: 0, grayed: index !== 1, touchable: index === 1 },
						})),
					},
				},
				{
					kind: 'addGear',
					selector: { ...targets.claimedMark.target.selector, kind: 'display', controllerName },
