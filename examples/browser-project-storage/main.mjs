...
export async function createBrowserExample() {
	if (!navigator.storage?.getDirectory || !navigator.locks)
		throw new Error('This example requires OPFS and Web Locks on localhost or HTTPS.');
	...
	await navigator.locks.request('openfairygui-example:initialize', async () => {
		...
		const component = document
			...
			.setSize(320, 180);
		component.addChild(
			document.createGTextField('title').setId('title').setText('Hello browser').setXY(16, 18).setSize(240, 32),
		);
		pkg.addResource(component);
		pkg.addResource(
			document
				.createImageResource('pixel')
				.setId('pixel')
				.setPath('/')
				.setFileName('pixel.png')
				.setWidth(2)
				.setHeight(1),
		);
		...
		project.packages[0].resources.find((resource) => resource.kind === 'image').sourceBytes = new Uint8Array(
	...
	function read() {
		...
		const title = component?.component?.displayList.find(
			(entry) => entry.kind === 'text' && entry.name === 'title',
		);
	...
	return {
		...
		async edit(text) {
			...
			const transaction = {
				...
				operations: [{ kind: 'setDisplayNodeProps', selector: current.selector, props: { text } }],
			};
		...
		async validate() {
			return validateProjectWeb(await readProjectAsUam(io, projectPath, { hydrateResourceBytes: true }));
		},
	};
}
// #endregion example
			...
			output.textContent = JSON.stringify(
				{
					...
					...(current
						? {
								title: current.entity.properties.text,
								revision: current.revision,
								dirty: data(example.runtime.getSession({ sessionId: example.sessionId })).dirty,
							}
						: { session: 'closed' }),
