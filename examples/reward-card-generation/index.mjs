...
export async function createRewardCardProject(parent) {
	...
	const template = document
		.createComponent('RewardCardTemplate')
		.setId('cardtmpl')
		.setPath('/')
		.setSize(240, 180)
		.setExtensionType('Label')
		.setExported(true);
	template.addChild(
		document
			.createGGraph('background')
			.setId('card-bg')
			.setSize(240, 180)
	...
	template.addChild(
		document
			...
			.setXY(88, 28)
			.setSize(64, 64)
			.setUrl('ui://pkgsharered')
			.setFill(LoaderFillType.Scale)
			.setTouchable(false),
	);
	template.addChild(
		document
			.createGTextField('title')
			.setId('card-title')
			.setXY(16, 124)
			.setSize(208, 32)
			.setText('奖励模板')
...
export async function generateRewardCards(
	projectPath,
	...
	try {
		...
		const shared = sharedPackages[0];
		const templates = main.resources.filter(
			(resource) => resource.kind === 'component' && resource.name === 'RewardCardTemplate',
		);
		...
		for (const card of cards) {
			assert(
				!main.resources.some((resource) => resource.id === card.id || resource.name === card.name),
				`Generated card already exists: ${card.name}; query and replan`,
			);
			const images = shared.resources.filter(
				(resource) => resource.kind === 'image' && resource.name === card.image,
			);
			assert.equal(images.length, 1, `Expected one Shared/${card.image} image`);
			const icon = data(
				await runtime.queryEntity({
					sessionId,
					target: { kind: 'resource', selector: { packageId: shared.id, resourceId: images[0].id } },
				}),
			);
			...
			wrapper.addChild(
				document
					.createGComponent('card')
					.setId('card')
					.setSrc(templates[0].id)
					.setPackageId(main.id)
					.setSize(width, height)
					.setInstanceExtType('Label')
					.setInstanceTitle(card.title)
					.setInstanceIcon(`ui://${shared.id}${images[0].id}`),
			);
		...
		const transaction = {
			...
			operations: resources.map((component, index) => ({
				kind: 'addComponent',
				selector: { packageId: main.id },
				component,
				atIndex: main.resources.length + index,
			})),
		};
		...
		return {
			projectPath,
			template: selector,
			generated: resources.map(({ id, name }) => ({ id, name })),
		...
		throw Object.assign(
			new Error('Reward card generation needs host recovery; the session remains open.', { cause }),
			{
...
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
	...
	if (values.create)
		console.log(JSON.stringify({ projectPath, target: 'Main/RewardCardTemplate', generated: false }, null, 2));
	else {
