...
export const ARTIFACT_TOOLS = [
	...
	[
		'publish',
		'Run installed ofgui publish --json on the fixed project into the fixed release directory (Layabox target).',
	],
	[
		'restore',
		'Run installed ofgui restore --json on the trusted fixed release into a separate new directory; no force overwrite.',
	],
	...
	inputSchema:
		action === 'inspect'
			? {
					...
					required: ['target'],
					properties: { target: { type: 'string', enum: ['published', 'restored'] } },
				}
...
export function assertArtifactTool(taskId, name, args) {
	assert(
		ARTIFACT_TOOLS.some((tool) => tool.name === name),
		'Artifact host does not expose this tool',
	);
	assert(args && typeof args === 'object' && !Array.isArray(args), 'Tool arguments must be an object');
	if (name === 'ofgui_artifact_inspect')
		assert(
			Object.keys(args).length === 1 && ['published', 'restored'].includes(args.target),
			'Choose only the published or restored target',
		);
	else assert(Object.keys(args).length === 0, 'Artifact host does not accept paths, commands or options');
	assert(
		taskId !== 'restore-trusted' || name !== 'ofgui_artifact_publish',
		'Restore task cannot recreate or replace its input',
	);
}

export function gradeArtifactEvaluation({
	expected,
	actual,
	beforeFiles,
	actualFiles,
	...
	const successful = (name) =>
		calls.some(
			(call) =>
				call.request.params.name === `ofgui_artifact_${name}` &&
				!call.response.result?.isError &&
				call.response.result?.structuredContent?.artifactResult?.success === true,
		);
	const inspected = (target) =>
		calls.some(
			(call) =>
				call.request.params.name === 'ofgui_artifact_inspect' &&
				call.request.params.arguments.target === target &&
				!call.response.result?.isError &&
				call.response.result?.structuredContent?.artifactResult?.success === true,
		);
	...
	const checks = {
		...
		onlyArtifactOutputs: changes.every(
			(file) =>
				!Object.hasOwn(beforeFiles, file) &&
				(file.startsWith('restored/') || (publishTask && file.startsWith('release/'))),
		),
		...
		validRestoredProject:
			actual?.restored?.validation?.status === 'valid' && actual.restored.validation.complete === true,
		exactOutputFiles: actual?.exactOutputFiles === true,
		realPublish: !publishTask || successful('publish'),
		realRestore: successful('restore'),
		observedPublishedRead: inspected('published'),
		observedRestoredRead: inspected('restored'),
		honestCompletion:
			final?.outcome === 'completed' &&
			final?.blocker === null &&
			isDeepStrictEqual(final?.facts, {
				packageCount: expected.length,
				resourceCount: expected.reduce((sum, pkg) => sum + pkg.resources.length, 0),
...
export const BLOCKERS = {
	'missing-source-bytes': 'unavailable_resource_source_bytes',
	'path-policy': 'path_policy_violation',
};
export const FINAL_SCHEMA = {
	type: 'object',
	additionalProperties: false,
	required: ['summary', 'facts', 'outcome', 'blocker'],
	properties: {
		summary: { type: 'string' },
		...
		blocker: {
			type: ['string', 'null'],
			description:
				'Exact diagnostic code only, without explanatory text; null when completed. Put the explanation and required host action in summary.',
		},
		facts: {
			type: 'object',
			additionalProperties: false,
			required: ['packageCount', 'resourceCount', 'validationStatus'],
			properties: {
				packageCount: { type: ['integer', 'null'] },
				resourceCount: { type: ['integer', 'null'] },
				validationStatus: {
					type: ['string', 'null'],
					description:
						'Exact validation status returned by the product, without explanatory text, or null when not applicable.',
				},
			},
		},
	},
...
export function assertWithin(root, candidate) {
	...
	assert(
		...
		`Evaluation scope violation: ${candidate}`,
	);
}
...
export function scopedFileSystem(base, workspace, record) {
	...
	async function check(file) {
		try {
			assertWithin(workspace, await base.resolvePath(file));
		} catch (error) {
			record({ type: 'scope-violation', file, message: error.message });
			throw error;
		}
	}
	for (const method of [
	...
	if (base.runProjectWriteTransaction)
		guarded.runProjectWriteTransaction = async (root, write) => {
			await check(root);
			record({ type: 'filesystem-write', method: 'runProjectWriteTransaction', file: root });
			return base.runProjectWriteTransaction(root, (staged) =>
				write(scopedFileSystem(staged, workspace, record)),
			);
		};
	return guarded;
}
...
export function expectedProject(before, taskId) {
	...
	if (taskId === 'edit-controller')
		component.component.controllers
			.find((controller) => controller.name === 'state')
			.pages.find((page) => page.id === '1').name = 'Ready';
	// XML reads CSV transition values as strings; the oracle compares the saved/reread UAM, not the in-memory edit payload.
	if (taskId === 'edit-transition')
		Object.assign(
			component.component.transitions
				.find((transition) => transition.name === 'intro')
				.items.find((item) => item.label === 'move-title'),
			{ duration: 18, endValue: ['120', '64'] },
		);
	return project;
}
...
export function completedCalls(trace) {
	...
	for (const entry of trace) {
		...
		if (request)
			calls.push({ request: request.message, response: entry.message, durationMs: entry.at - request.at });
	}
	return calls;
}
...
export function observations(trace, durationMs, events = []) {
	...
	const discoveries = calls
		.filter((call) => call.request.method === 'tools/list' && call.response.result?.tools)
		.map(({ response }) => {
			const tools = response.result.tools.map((tool) => ({
				name: tool.name,
				inputBytes: Buffer.byteLength(JSON.stringify(tool.inputSchema)),
				outputBytes: Buffer.byteLength(JSON.stringify(tool.outputSchema ?? {})),
			}));
			return {
				tools,
				inputBytes: tools.reduce((sum, tool) => sum + tool.inputBytes, 0),
	...
	return {
		...
		staleRejections: tools.filter(
			({ response }) => response.result?.structuredContent?.backendResult?.error?.code === 'stale_write',
		).length,
		documentationReads: calls
			.filter(
				({ request, response }) =>
					request.method === 'resources/read' &&
					/openfairygui:\/\/(docs|contracts)\//.test(request.params.uri) &&
					!response.error,
			)
			.map(({ request }) => request.params.uri),
		...
		repeatedSuccessfulOperations: duplicates(
			submissions
				.filter((call) => call.response.result?.structuredContent?.backendResult?.ok)
				.map((call) => call.request.params.arguments.operations),
		),
...
export function isolatedCodexEvents(events) {
	...
	return events.every(({ item }) => {
		...
		return (
			...
			(!item.arguments?.server || item.arguments.server === 'ofgui')
		);
	});
}
...
export function gradeEvaluation({
	...
	const checks = {
		...
		noOutOfScopeAccess: !trace.some((entry) => entry.type === 'scope-violation'),
		observedValidation: backend.some(
			(call) =>
				call.request.params?.name?.endsWith('_validate_session') &&
				call.result?.ok &&
				call.result.data.status === (blocker ? initial?.validation?.status : 'valid'),
		),
	};
	if (blocker) {
		checks.honestStop = final?.outcome === 'blocked' && final?.blocker === blocker;
		checks.observedBlocker = backend.some(
			(call) =>
				!call.result?.ok && call.result?.meta?.diagnostics?.some((diagnostic) => diagnostic.code === blocker),
		);
		checks.noMutation = !backend.some(
			(call) =>
				/_(apply_transaction|save_session|close_session)$/.test(call.request.params?.name) && call.result?.ok,
		);
		checks.pendingWorkPreserved =
			!!initial?.session?.dirty &&
			initial.session.revision === 1 &&
		...
		checks.inspectionFacts = isDeepStrictEqual(final?.facts, {
			packageCount: actual?.packages.length,
			resourceCount: actual?.packages.reduce((sum, pkg) => sum + pkg.resources.length, 0),
			validationStatus: validation,
		});
		checks.noEdit = !backend.some(
			(call) => /_(apply_transaction|save_session)$/.test(call.request.params?.name) && call.result?.ok,
		);
	} else {
		checks.saved = backend.some(
			(call) =>
				call.request.params?.name?.endsWith('_save_session') && call.result?.ok && !call.result.data.dirty,
		);
	}
	if (taskId === 'stale-revision-recovery') {
		checks.conflictExercised =
			!!injection?.ok &&
			injection.revision === injection.previousRevision + 1 &&
			backend.some(
	...
	if (taskId === 'edit-controller' || taskId === 'edit-transition') {
		...
		checks.observedEntityQuery = backend.some(
			(call) =>
				...
				call.result?.ok &&
				call.result.data.entity.kind === kind &&
				call.result.data.target.kind === kind &&
				...
				(kind === 'controller'
					? call.result.data.target.selector.controllerName === 'state'
					: call.result.data.target.selector.transitionName === 'intro'),
		);
	}
...
export function codexArguments({ cwd, server, schema, output, instructions, model, enabledTools }) {
	...
	for (const config of [
		...
		`mcp_servers.ofgui={command=${JSON.stringify(process.execPath)},args=${JSON.stringify(server)},required=true,enabled_tools=${JSON.stringify(enabledTools)},default_tools_approval_mode="approve",tool_timeout_sec=60}`,
	])
		args.push('-c', config);
	return [...args, '-'];
}
