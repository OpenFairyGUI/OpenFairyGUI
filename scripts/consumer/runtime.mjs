...
function cli(args) {
	try {
		const output = execFileSync(...bin('ofgui', '@openfairygui/cli', args), {
			cwd: root,
			encoding: 'utf8',
			timeout: 30_000,
		});
		...
		await new Promise((resolve, reject) => {
						...
						if (response.id === 1) {
							...
							assert(
								response.result.tools.every(
									(tool) => tool._meta?.['openfairygui/contractDigest'] === expectedCatalog.digest,
								),
							);
							send({
								id: 3,
								method: 'resources/read',
								params: { uri: 'openfairygui://contracts/operations' },
							});
							...
							send({
								id: 4,
								method: 'resources/read',
								params: { uri: 'openfairygui://contracts/operations/addComponent' },
							});
							...
							send({
								id: 7,
								method: 'resources/read',
								params: { uri: 'openfairygui://docs/diagnostics/stale_write' },
							});
			...
			send({
				...
				params: {
					...
					capabilities: {},
					clientInfo: { name: 'tarball-consumer', version: '1.0.0' },
				},
			});
		});
	} finally {
...
async function hostCompositionSmoke() {
	...
	runtime.saveSession = async (input) => {
		saveCalls++;
		return (backendResult = await saveSession(input));
	};
	const failure = {
		ok: false,
		error: { code: 'save_approval_required', approvalRequestId: 'owner-request', approvalPath: '/#save-approvals' },
	};
	const policy = {
	...
	const server = createOpenFairyGuiMcpServer({
		runtime,
		instructions: 'Host writes require owner approval.',
		toolPolicies: {
			openfairygui_backend_save_session: policy,
			openfairygui_backend_materialize_session: policy,
		},
	});
	...
	try {
			...
			!(
				await call('apply_transaction', {
					...
					operations: [
						{
							kind: 'renameResource',
							selector: { packageId: 'pkgdemo1', resourceId: 'cmpdemo1' },
							newName: 'Approved',
						},
					],
				})
		...
		for (const method of ['save_session', 'materialize_session']) {
			...
			assert(
				z
					...
					.safeParse(pending.structuredContent).success,
			);
		}
		assert.equal(saveCalls, 0);
		assert.deepEqual(snapshot(path.dirname(projectPath)), before);
		...
		assert.equal(
			(await call('save_session', input)).structuredContent.backendResult.error.code,
			'save_approval_required',
		);
		assert.equal(saveCalls, 1);
		grant = true;
		...
		assert.deepEqual(stale.structuredContent, { backendResult });
		assert.equal(saveCalls, 2);
		console.log(
			'[consumer] Host composition PASS: SDK discovery, installed docs/prompts, declared approval failure, one approved save and original Backend errors',
		);
	} finally {
...
async function sessionReadSmoke() {
	...
	for (const mode of ['sdk', 'mcp']) {
		...
		try {
			if (client) {
				await client.connect(
					new StdioClientTransport({
						...
						env: { OPENFAIRYGUI_ALLOWED_PROJECT_ROOTS: projectRoot },
						stderr: 'inherit',
					}),
				);
			...
			assert.equal(initial.data.dirty, false);
			const applied = await call('applyTransaction', {
				sessionId,
				expectedRevision: 0,
				operations: [
					{
						kind: 'setDisplayNodeProps',
						selector: { packageId: 'pkgdemo1', componentResourceId: 'cmpdemo1', displayNodeId: 'title' },
						props: { text: 'unsaved consumer read' },
					},
					{ kind: 'replaceResourceBytes', selector, sourceBytes: runtime ? replacement : [...replacement] },
				],
			});
			assert(applied.ok, JSON.stringify(applied));
			const state = await call('readSessionState', { sessionId, expectedRevision: applied.data.revision });
			assert(state.ok, JSON.stringify(state));
			assert.equal(state.data.revision, 1);
			assert.equal(state.data.lastSavedRevision, 0);
			assert.equal(state.data.dirty, true);
			assert.equal(state.data.uamFidelity, 'full');
			assert.equal(state.data.readComplete, true);
			assert.deepEqual(state.data.readDiagnostics, []);
			const main = state.data.project.packages
				.find((pkg) => pkg.id === 'pkgdemo1')
				.resources.find((resource) => resource.id === 'cmpdemo1');
			assert.equal(main.component.displayList.find((node) => node.id === 'title').text, 'unsaved consumer read');
			for (const pkg of state.data.project.packages)
				for (const resource of pkg.resources) assert(!Object.hasOwn(resource, 'sourceBytes'));
			const bytes = await call('readResourceBytes', {
				sessionId,
			...
			for (const method of ['readSessionState', 'readResourceBytes']) {
				const stale = await call(method, {
					sessionId,
					expectedRevision: 0,
					...(method === 'readResourceBytes' ? { selector } : {}),
				});
				assert.equal(stale.ok, false);
				assert.equal(stale.error.code, 'stale_read');
				assert.equal(stale.error.actualRevision, 1);
			}
			const after = await call('getSession', { sessionId });
			assert(after.ok);
			assert.deepEqual(after.data, applied.data);
			assert.deepEqual(
				snapshot(projectRoot),
				beforeFiles,
				`${mode} reads must expose unsaved bytes without saving or changing source files`,
			);
		} finally {
			try {
				if (sessionId) await call('closeSession', { sessionId });
			} finally {
...
async function rewardPanelSmoke() {
	...
	for (const mode of ['sdk', 'mcp']) {
		...
		try {
			if (client) {
				await client.connect(
					new StdioClientTransport({
						...
						env: { OPENFAIRYGUI_ALLOWED_PROJECT_ROOTS: allowedProjectRoots.join(path.delimiter) },
						stderr: 'inherit',
					}),
				);
				await client.listTools();
			...
			const runtime = client
				? Object.fromEntries(
						OPENFAIRYGUI_BACKEND_TOOL_DEFINITIONS.map((tool) => [
							tool.backendMethod,
							async (input) => {
			...
			assert.deepEqual(result.preview.impact.files, [
				{ path: 'assets/Main/RewardPanel.xml', kind: 'file', change: 'updated' },
			]);
			const after = await readProjectAsUam(new NodeIO(), projectPath);
			assert.deepEqual(result.project, after);
			const expected = structuredClone(before);
			const panel = expected.packages
				.find((pkg) => pkg.name === 'Main')
				.resources.find((resource) => resource.name === 'RewardPanel').component;
			panel.controllers.push({
				name: 'rewardState',
				selectedIndex: 0,
			...
			panel.displayList
				.find((node) => node.name === 'claimButton')
				.gears.push(
					{
						...gearDefaults,
						kind: 'look',
						defaultValue: { alpha: 1, rotation: 0, grayed: true, touchable: false },
						states: [
					...
					},
					{
						...gearDefaults,
						kind: 'text',
						defaultValue: { text: '未达成' },
						states: [
							{ pageId: 'locked', value: { text: '未达成' } },
							{ pageId: 'claimable', value: { text: '领取奖励' } },
							{ pageId: 'claimed', value: { text: '已领取' } },
						],
					},
				);
			panel.displayList
				.find((node) => node.name === 'claimedMark')
				.gears.push({
					kind: 'display',
					name: '',
					controllerName: 'rewardState',
					visibleOnPageIds: ['claimed'],
				});
			assert.deepEqual(
				after,
				expected,
				'Only the controller and three gears may change; preserve all other semantics and bytes',
			);
			const afterFiles = snapshot(directory);
			assert.deepEqual(Object.keys(afterFiles).sort(), Object.keys(beforeFiles).sort());
			assert.deepEqual(
				Object.keys(afterFiles).filter((file) => afterFiles[file] !== beforeFiles[file]),
				['assets/Main/RewardPanel.xml'],
			);
			...
			assert(reopened.ok);
			const target = await runtime.queryEntity({
				sessionId: reopened.data.sessionId,
				target: { kind: 'controller', selector: { ...result.selector, controllerName: 'rewardState' } },
			});
			assert(target.ok);
			assert.deepEqual(target.data.entity.properties, panel.controllers.at(-1));
			...
			assert.equal(redesigned.revision, 1);
			assert.equal(redesigned.dirty, false);
			assert.equal(redesigned.validation.status, 'valid');
			assert.equal(redesigned.validation.complete, true);
			assert.deepEqual(redesigned.preview.impact.files, [
				{ path: 'assets/Main/RewardPanel.xml', kind: 'file', change: 'updated' },
			]);
			const layoutExpected = structuredClone(after);
			const layoutPanel = layoutExpected.packages
				.find((pkg) => pkg.name === 'Main')
				.resources.find((resource) => resource.name === 'RewardPanel').component;
			...
			for (const [name, props] of Object.entries(expectedLayout))
				Object.assign(
					layoutPanel.displayList.find((node) => node.name === name),
					props,
				);
			...
			assert.deepEqual(
				redesigned.project,
				layoutExpected,
				'Only B layout and intro may change; all A controller/gear semantics remain',
			);
			const redesignedFiles = snapshot(directory);
			assert.deepEqual(Object.keys(redesignedFiles).sort(), Object.keys(afterFiles).sort());
			assert.deepEqual(
				Object.keys(redesignedFiles).filter((file) => redesignedFiles[file] !== afterFiles[file]),
				['assets/Main/RewardPanel.xml'],
			);
			runtime.preflightTransaction = preflight;
			...
			const layoutReopened = await runtime.openSession({ projectPath });
			assert(layoutReopened.ok);
			const transition = await runtime.queryEntity({
				sessionId: layoutReopened.data.sessionId,
				target: { kind: 'transition', selector: { ...redesigned.selector, transitionName: 'intro' } },
			});
			assert(transition.ok);
			assert.deepEqual(transition.data.entity.properties, layoutPanel.transitions.at(-1));
			assert((await runtime.closeSession({ sessionId: layoutReopened.data.sessionId })).ok);
			const published = await publishRewardPreview(
				projectPath,
				path.join(root, `reward-layout-${mode}-published`),
			);
			const binary = await new NodeIO().readBinary(
				published.files.find((file) => path.basename(file.path) === 'Main.fui').path,
			);
			const publishedPanel = binary
				.getRoot()
				.listPackages()
				.find((pkg) => pkg.getName() === 'Main')
				.listComponents()
				.find((component) => component.getName() === 'RewardPanel');
			...
			for (const node of publishedPanel.listChildren()) {
				...
				assert.deepEqual(
					...
					[
						expectedNode.position.x,
						expectedNode.position.y,
						expectedNode.size.width,
						expectedNode.size.height,
					],
				);
				if (node.getAutoSize)
					assert.equal(
						node.getAutoSize(),
						0,
						'Fixed text boxes must retain their width and center alignment',
					);
			}
			const intro = publishedPanel.listTransitions().find((item) => item.getName() === 'intro');
			assert(intro.getAutoPlay());
			assert.equal(intro.getAutoPlayTimes(), 1);
			assert.equal(intro.listItems().length, 2);
			for (const [index, item] of intro.listItems().entries()) {
				assert.equal(item.getTargetId(), '');
				assert.equal(item.getActionType(), index === 0 ? 4 : 0);
				assert.equal(item.getTime(), 0);
				assert(
					Math.abs(item.getDuration() / intro.getFps() - 0.4) < 0.000001,
					'Published timing must be seconds, not UAM frames',
				);
			...
			assert.deepEqual(
				snapshot(directory),
				redesignedFiles,
				'Publishing outside the project must preserve source files',
			);
	...
	console.log(
		'[consumer] Reward panels PASS: SDK and real MCP; states, layout and intro; full UAM/PNG preservation; duplicate rejection/reopen; published 0.4s timing',
	);
}
...
async function rewardCardSmoke(projectPath, runtime, mode) {
	...
	runtime.preflightTransaction = async (input) => {
		assert.deepEqual(
			input.operations.map((operation) => operation.kind),
	...
	assert.deepEqual(result.preview.impact.files, [
		...addedFiles.map((file) => ({ path: file, kind: 'file', change: 'added' })),
	...
	assert.deepEqual(
		preserved,
		before,
		'Preserve the entire template, unrelated UAM, resource references and hydrated bytes',
	);
	assert.equal(generated.length, 3);
	for (const [index, resource] of generated.entries()) {
		const card = expected[index];
		assert.deepEqual(
			[resource.id, resource.name, resource.kind, resource.path, resource.exported, resource.branch],
			[card.id, card.name, 'component', '/', true, ''],
		);
		assert.deepEqual(resource.component.size, { width: 240, height: 180 });
		assert.deepEqual(resource.component.controllers, []);
		assert.deepEqual(resource.component.transitions, []);
		assert.equal(
			resource.component.displayList.length,
			1,
			'A generated wrapper must reference the template, not copy its children',
		);
		...
		assert.deepEqual(node.size, { width: 240, height: 180 });
		assert.deepEqual(node.gears, []);
		assert.deepEqual(node.relations, []);
		assert.deepEqual(node.instanceProperties, {
			extensionType: 'Label',
			title: card.title,
			icon: card.icon,
			titleColor: '',
	...
	assert.deepEqual(
		Object.keys(afterFiles)
			.filter((file) => !Object.hasOwn(beforeFiles, file))
			.sort(),
		addedFiles,
	);
	assert.deepEqual(
		Object.keys(beforeFiles).filter((file) => afterFiles[file] !== beforeFiles[file]),
		['assets/Main/package.xml'],
	);
	await assert.rejects(() => generateRewardCards(projectPath, runtime), /Generated card already exists/);
	assert.deepEqual(
		snapshot(directory),
		afterFiles,
		'Duplicate generation must preserve saved files and release the clean-session lock',
	);
	const reopened = await runtime.openSession({ projectPath });
	assert(reopened.ok);
	try {
		for (const resource of generated) {
			const node = await runtime.queryEntity({
				sessionId: reopened.data.sessionId,
				target: {
					kind: 'displayNode',
			...
			assert(node.ok);
			assert.deepEqual(node.data.entity.properties, resource.component.displayList[0]);
		}
	} finally {
		assert((await runtime.closeSession({ sessionId: reopened.data.sessionId })).ok);
	}
	...
	for (const card of expected) {
		...
		assert.deepEqual(
			[
				child.getSrc(),
				child.getPackageId(),
				child.getInstanceExtType(),
				child.getInstanceTitle(),
				child.getInstanceIcon(),
			],
			['cardtmpl', '', 'Label', card.title, card.icon],
		); // Same-package binary references omit the package ID.
	}
	assert.deepEqual(
		snapshot(directory),
		afterFiles,
		'Publishing must preserve the original template and all source files',
	);
	console.log(
		`[consumer] Reward cards ${mode} PASS: three components, shared template/PNG preservation, preview, duplicate rejection/reopen and published references`,
	);
}
...
export async function runtimeSmoke() {
	...
	for (const source of expected) {
		...
		for (const range of Object.values({ ...manifest.dependencies, ...manifest.optionalDependencies }))
			assert(!/^(workspace:|link:|file:)/.test(range), 'Unpublishable dependency');
		...
		for (const [subpath, conditions] of Object.entries(manifest.exports ?? {})) {
			...
			assert(Object.keys(await import(specifier)).length > 0, `Empty ESM entry: ${specifier}`);
			esmCount++;
			if (conditions.require) {
				contained(directory, require.resolve(specifier));
				assert(Object.keys(require(specifier)).length > 0, `Empty CJS entry: ${specifier}`);
				cjsCount++;
			}
		}
	}
	...
	assert.equal(mcpExample.session.dirty, false);
	assert.equal(mcpExample.current.revision, 0);
	assert.equal(mcpExample.preview.projectedRevision, 1);
	assert.deepEqual(mcpExample.preview.impact.files, [
		{ path: 'assets/Main/MainView.xml', kind: 'file', change: 'updated' },
	]);
	...
	assert.equal(report.validation.status, 'valid');
	assert.equal(report.validation.complete, true);
	assert.equal(report.inspection.totals.packages, 1);
	assert.equal(report.inspection.totals.components, 1);
	...
	assert.equal(
		JSON.parse(cli(['backend-capabilities', projectRoot, '--json'])).result.runtimeOwner,
	...
	assert.equal(
		expectedDocs.index.packageVersion,
		expected.find((entry) => entry.name === '@openfairygui/backend').version,
	);
	...
	for (const [id, file] of [
		...
		['skill', 'docs/skills/openfairygui/SKILL.md'],
	]) {
	...
	assert(
		JSON.parse(cli(['docs', 'find', 'stale_write', '--json'])).result.documents.some(
			(entry) => entry.id === 'diagnostics/stale_write',
		),
	);
	assert.deepEqual(
		JSON.parse(JSON.parse(cli(['docs', 'schema', 'setDisplayNodeProps', '--json'])).result.text),
		docs.getOpenFairyGuiOperationSchema('setDisplayNodeProps'),
	);
	for (const id of ['../package.json', 'constructor', 'methods/unknown']) {
		assert.throws(
			() => cli(['docs', 'cat', id, '--json']),
			(error) => error.status === 1 && JSON.parse(error.stdout).error.code === 'documentation_unavailable',
		);
	}
	const doctor = JSON.parse(cli(['doctor', projectRoot, '--json'])).result;
	assert.equal(doctor.status, 'ready');
	assert.equal(doctor.project.status, 'valid');
	assert(doctor.project.complete);
	assert.equal(doctor.packageVersion, doctor.cliVersion);
	assert.deepEqual(
		doctor.checks.map((check) => [check.id, check.status]),
		[
			['native-images', 'ok'],
			['temp-directory', 'ok'],
		],
	...
	assert.equal(noProject.scope, 'installed-product');
	const outputCheck = noProject.checks.find((check) => check.id === 'output-directory');
	assert.equal(outputCheck.path, outputDirectory);
	assert.equal(outputCheck.inspectedPath, realpathSync.native(projectRoot));
	assert.equal(outputCheck.status, 'ok');
	assert.equal(outputCheck.exists, false);
	assert(!existsSync(path.dirname(outputDirectory)), 'Doctor must not create a missing output directory');
	assert.throws(
		() => cli(['doctor', '--output-dir', projectPath, '--json']),
		(error) =>
			error.status === 1 &&
			JSON.parse(error.stdout).result.checks.some(
				(check) => check.id === 'output-directory' && check.status === 'error',
			),
	);
	assert.throws(
		() => cli(['doctor', '--output-dir=', '--json']),
		(error) => error.status === 2 && JSON.parse(error.stdout).error.code === 'invalid_arguments',
	);
	// Exercise a broken temp path through the installed CLI, without a source loader needing its own temp cache.
	const badTemp = spawnSync(...bin('ofgui', '@openfairygui/cli', ['doctor', '--json']), {
		cwd: root,
		encoding: 'utf8',
		timeout: 30_000,
	...
	unsupportedProject.packages[0].resources.find(
		(resource) => resource.kind === 'component',
	).component.displayList[0].name = 'not-title';
	...
	await assert.rejects(
		editAndSave(unsupportedPath),
		/This example expects Main\/MainView with a text node named title/,
	);
	assert.deepEqual(
		snapshot(path.dirname(unsupportedPath)),
		unsupportedBefore,
		'Examples failing before apply must preserve the source',
	);
	...
	decoderDocument
		...
		.listPackages()[0]
		.addResource(
			decoderDocument.createImageResource('pixel.png').setId('pixel').setPath('/').setFileName('pixel.png'),
		);
	const decoderProject = liftDocumentToUamProject(decoderDocument);
	decoderProject.packages[0].resources.find((entry) => entry.kind === 'image').sourceBytes = Uint8Array.from(
		Buffer.from(
	...
	const incomplete = spawnSync(...bin('ofgui', '@openfairygui/cli', ['doctor', decoderProjectPath, '--json']), {
		cwd: root,
		encoding: 'utf8',
		timeout: 30_000,
		env: { ...process.env, NODE_OPTIONS: `--import=${register}` },
	});
	...
	assert.equal(incompleteReport.project.complete, false);
	...
	const noDecoder = spawnSync(...bin('ofgui', '@openfairygui/cli', ['doctor', '--json']), {
		cwd: root,
		encoding: 'utf8',
		timeout: 30_000,
		env: { ...process.env, NODE_OPTIONS: `--import=${register}` },
	...
	assert.equal(noDecoderEnvelope.result.project, null);
	assert.equal(noDecoderEnvelope.result.checks[0].status, 'incomplete');
	assert.deepEqual(snapshot(path.dirname(decoderProjectPath)), decoderBefore);
	assert.deepEqual(
		snapshot(projectRoot),
		beforeFiles,
		'Product diagnosis and documentation must not change project files',
	);
	assert.equal(
		execFileSync(...bin('openfairygui', '@openfairygui/cli', ['--version']), { encoding: 'utf8' }).trim(),
		expected[0].version,
	);
	...
	assert.equal(edited.dirty, false);
	const target = beforeProject.packages[0].resources.find((entry) => entry.kind === 'component').component
		.displayList[0];
	...
	assert.deepEqual(
		Object.keys(afterFiles).filter((file) => beforeFiles[file] !== afterFiles[file]),
		['assets/Main/MainView.xml'],
	);
	assert.equal(JSON.parse(cli(['validate', projectRoot, '--json'])).result.status, 'valid');
	...
	try {
		assert.equal(
			JSON.parse(cli(['doctor', projectRoot, '--json'])).result.status,
			'ready',
			'Doctor must not contend with a session lock',
		);
		const component = beforeProject.packages[0].resources.find((entry) => entry.kind === 'component');
		const queried = runtime.queryEntity({
			sessionId: reopened.data.sessionId,
			target: {
				kind: 'displayNode',
				selector: {
					packageId: beforeProject.packages[0].id,
					componentResourceId: component.id,
					displayNodeId: target.id,
				},
			},
		});
	...
	for (const failureKind of ['invalid', 'incomplete', 'valid-but-incomplete', 'save']) {
		...
		let saveCalls = 0;
		fileSystem.runProjectWriteTransaction = (directory, write) =>
			atomicWrite(directory, (staged) =>
				write({
					...staged,
					async writeFile(file, content) {
						if (injectFailure && failureKind === 'save')
							throw Object.assign(new Error('Injected consumer write failure'), { code: 'EACCES' });
		...
		recoveryRuntime.validateSession = (input) => {
			...
			assert(result.ok);
			return injectFailure && failureKind !== 'save'
				? {
						...result,
						data: {
							...result.data,
							status: failureKind === 'valid-but-incomplete' ? 'valid' : failureKind,
							complete: failureKind === 'invalid',
						},
					}
				: result;
		};
		recoveryRuntime.saveSession = (input) => {
			saveCalls++;
		...
		try {
			...
			assert.equal(retained.data.dirty, true);
			assert.equal(retained.data.revision, 1);
			assert.equal(retained.data.lastSavedRevision, 0);
			assert.equal(retained.data.lockHeld, true);
			assert.equal(
				saveCalls,
				failureKind === 'save' ? 1 : 0,
				'No save after failed validation or automatic retry after a write failure',
			);
			assert.deepEqual(
				snapshot(recoveryRoot),
				originalFiles,
				'Failed validation or staged writes must preserve the original files',
			);
			if (failureKind === 'save') {
				assert.equal(failure.cause.cause.error.code, 'save_partial_failure');
				assert.equal(
					failure.cause.cause.error.diskMayBePartiallyUpdated,
					false,
					'Root runtime recognizes rollback from the separately bundled Node adapter',
				);
			...
			const peer = await createNodeBackendRuntime({ allowedProjectRoots: [recoveryRoot] }).openSession({
				projectPath: recoveryPath,
			});
			...
			assert(saved.ok && !saved.data.dirty);
			assert.equal(saved.data.revision, 1);
			const restored = await readProjectAsUam(new NodeIO(), recoveryPath);
			assert.equal(
				restored.packages[0].resources.find((resource) => resource.kind === 'component').component
					.displayList[0].text,
				'Recover the unsaved edit',
			);
		} finally {
			assert((await recoveryRuntime.closeSession({ sessionId })).ok);
		}
	...
	console.log(
		'[consumer] Edit recovery PASS: failed validation/write retain revision, dirty state, diagnostics and lock; explicit recovery saves the same session',
	);
	...
	for (const name of [
		'node-inspect-validate',
		'revision-checked-edit-save',
		'publish-restore',
		'mcp-stdio-client',
		'reward-panel-states',
		'reward-panel-layout',
	...
	console.log(
		`[consumer] Runtime PASS: ${esmCount} ESM / ${cjsCount} CJS entries; CLI JSON; MCP discovery; read/edit/save/reread`,
	);
}

if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
	runtimeSmoke().catch((error) => {
		console.error(error);
		process.exitCode = 1;
	});
}
