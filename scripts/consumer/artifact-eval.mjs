...
import {
	...
	IMAGE_BYTES,
	mergePublishedPackages,
	publishAndRestore,
	supportedSemantics,
} from './examples/publish-restore/index.mjs';
import { assertCliEnvelope, bin, json, snapshot } from './helpers.mjs';
import {
	ARTIFACT_TOOLS,
	assertArtifactTool,
	assertWithin,
	gradeArtifactEvaluation,
	observations,
...
const facts = (semantics) => ({
	packageCount: semantics.length,
	resourceCount: semantics.reduce((sum, pkg) => sum + pkg.resources.length, 0),
	validationStatus: 'valid',
});

export function artifactCommand(args, record = () => {}) {
	const child = spawnSync(...bin('ofgui', '@openfairygui/cli', args), {
		cwd: root,
		encoding: 'utf8',
		timeout: 30_000,
		windowsHide: true,
...
function publishedCommand(config, record) {
	return artifactCommand(
		['publish', config.projectPath, '-o', config.release, '--project-type', 'layabox', '--json'],
		record,
	);
}
function restoredCommand(config, record) {
	return artifactCommand(
		['restore', config.release, '-o', config.restored, '--project-type', 'layabox', '--json'],
		record,
	);
}
...
async function pixelsMatch(image, color) {
	...
	return (
		...
		info.height === 2 &&
		info.channels === 4 &&
		data.equals(Buffer.from([...rgba, ...rgba, ...rgba, ...rgba]))
	);
}
...
async function inspectPublished(config) {
	...
	const seen = new Set();
	let pixels = true;
	for (const file of readdirSync(config.release)
		.filter((name) => name.endsWith('.fui'))
		.sort()) {
		...
		for (const pkg of document.getRoot().listPackages())
			for (const atlas of pkg.listAtlases()) {
				...
				assert.equal(metadata.width, atlas.getWidth());
				assert.equal(metadata.height, atlas.getHeight());
				for (const sprite of atlas.listSprites()) {
					assert(Object.hasOwn(IMAGE_BYTES, sprite.getItemId()), 'Unexpected fixture sprite');
					assert(!seen.has(sprite.getItemId()), 'Duplicate fixture sprite');
					seen.add(sprite.getItemId());
					let image = sharp(atlasPath).extract({
...
async function inspectRestored(config) {
	...
	for (const image of images)
		pixels =
			Object.hasOwn(IMAGE_BYTES, image.id) && (await pixelsMatch(sharp(image.sourceBytes), image.id)) && pixels;
	return {
		semantics: supportedSemantics(document),
		pixelsMatch: pixels,
		validation: await validateProjectNode(restored.result.projectPath),
	};
}
...
async function exactOutputFiles(config) {
	...
	for (const file of published.result.files) {
		...
		manifest[path.relative(config.release, file.path).replaceAll('\\', '/')] = readFileSync(file.path).toString(
			'base64',
		);
	}
	...
	await writeProjectFromUam(
		new NodeIO(),
		await readProjectAsUam(new NodeIO(), projectPath, { hydrateResourceBytes: true }),
		path.join(reconstructed, path.basename(projectPath)),
	);
	assert.deepEqual(
		Object.keys(snapshot(config.restored)).sort(),
		Object.keys(snapshot(reconstructed)).sort(),
		'No extra files outside the reread project',
	);
	return true;
}
...
export async function serveArtifact(config) {
	...
	const server = new Server(
		{ name: 'openfairygui-artifact-evaluation-host', version: '1' },
		{ capabilities: { tools: {}, resources: {} } },
	);
	server.setRequestHandler(ListToolsRequestSchema, () => ({
		tools: ARTIFACT_TOOLS.filter(
			(tool) => config.task.id !== 'restore-trusted' || tool.name !== 'ofgui_artifact_publish',
		),
	}));
	server.setRequestHandler(ListResourcesRequestSchema, () => ({
		resources: getInstalledDocumentationIndex()
			.documents.filter((doc) => ['workflow', 'restore-limits'].includes(doc.id))
	...
	server.setRequestHandler(ReadResourceRequestSchema, ({ params }) => {
		const entry = getInstalledDocumentationIndex().documents.find(
			(doc) => doc.uri === params.uri && ['workflow', 'restore-limits'].includes(doc.id),
		);
		if (!entry) {
			record({ type: 'scope-violation', uri: params.uri });
			throw new Error('Only installed artifact documentation is available');
		}
	...
	server.setRequestHandler(CallToolRequestSchema, ({ params }) => {
		const operation = pending.then(async () => {
			try {
				assertArtifactTool(config.task.id, params.name, params.arguments ?? {});
			} catch (error) {
				record({ type: 'scope-violation', params });
				throw error;
			}
			let report;
			if (params.name === 'ofgui_artifact_publish') {
				report = publishedCommand(config, record);
				saveJson(config.publishReport, report);
			} else if (params.name === 'ofgui_artifact_restore') {
				report = restoredCommand(config, record);
				saveJson(config.restoreReport, report);
			} else if (params.name === 'ofgui_artifact_context')
				report = {
					success: true,
					result: {
						projectPath: config.task.id === 'publish-consume' ? config.projectPath : null,
				...
				report = {
					success: true,
					result:
						params.arguments.target === 'published'
							? await inspectPublished(config)
							: await inspectRestored(config),
				};
			return {
				isError: !report.success,
				content: [{ type: 'text', text: JSON.stringify(report) }],
				structuredContent: { artifactResult: report },
			};
		});
	...
	const transport = {
		async start() {
			wire.onmessage = (message) => {
				if (message.id !== undefined) record({ type: 'request', message });
...
export async function referenceArtifact(client, config) {
	...
	const call = async (action, args = {}) => {
		const report = (await client.callTool({ name: `ofgui_artifact_${action}`, arguments: args })).structuredContent
			.artifactResult;
	...
	saveJson(config.final, {
		outcome: 'completed',
		blocker: null,
		summary:
			'Reference self-check only. Supported runtime semantics recovered, not original XML, project identity or editor-local state.',
		facts: facts(result.semantics),
	});
...
export async function runArtifactCase(task, options, run) {
	...
	const config = {
		...
		restored: path.join(workspace, 'restored'),
		publishReport: path.join(directory, 'publish.json'),
		restoreReport: path.join(directory, 'restore.json'),
		trace: path.join(directory, 'mcp.jsonl'),
		final: path.join(directory, 'final.json'),
		closed: path.join(directory, 'host-closed.json'),
	...
	writeFileSync(
		path.join(path.dirname(projectPath), 'notes.txt'),
		'Unpublished editor notes: must not be invented by restore.\n',
	);
	if (task.id === 'restore-trusted') {
		const report = publishedCommand(config);
	...
	saveJson(path.join(directory, 'before.json'), { files: beforeFiles });
	saveJson(path.join(directory, 'expected.json'), {
		semantics: expected,
		scope: 'Supported component geometry/text/IDs, cross-package references and decoded RGBA images; not source XML or editor-local state.',
	});
	...
	let error;
	try {
		runner = await run(config, configPath);
	} catch (failure) {
	...
	let final = null;
	try {
		actual.published = await inspectPublished(config);
		actual.restored = await inspectRestored(config);
	...
	try {
		...
		trace = lines(config.trace);
		final = existsSync(config.final) ? json(config.final) : null;
	} catch (failure) {
		error ??= failure.stack ?? failure.message;
	}
	const grade = gradeArtifactEvaluation({
		expected,
		actual,
		beforeFiles,
		actualFiles,
		trace,
		final,
		runnerOk: runner.ok && !error,
		isolated: runner.isolated,
		publishTask: task.id === 'publish-consume',
	});
	const result = {
		taskId: task.id,
		runner: options.runner,
		...grade,
		observations: observations(trace, durationMs, runner.events),
		error: error ?? null,
		exitCode: runner.exitCode ?? null,
		timedOut: runner.timedOut ?? false,
		clientWarnings: runner.clientWarnings ?? [],
	};
	saveJson(path.join(directory, 'actual.json'), { ...actual, files: actualFiles });
...
export async function artifactSmoke() {
	...
	assert.deepEqual(
		Object.fromEntries(
			Object.entries(snapshot(directory)).filter(
				([name]) => !name.startsWith('release/') && !name.startsWith('restored/'),
			),
		),
		before,
	);
	const config = {
		...
		publishReport: path.join(root, 'smoke-publish.json'),
		restoreReport: path.join(root, 'smoke-restore.json'),
	};
	saveJson(config.publishReport, { success: true, result: example.published });
	saveJson(config.restoreReport, { success: true, result: example.restored });
	assert((await inspectPublished(config)).pixelsMatch);
	assert((await inspectRestored(config)).pixelsMatch);
	...
	config.release = path.join(directory, 'cli-release');
	config.restored = path.join(directory, 'cli-restored');
	const published = publishedCommand(config);
	assert(published.success);
	saveJson(config.publishReport, published);
	const restored = restoredCommand(config);
	assert(restored.success);
	saveJson(config.restoreReport, restored);
	...
	const corrupt = path.join(directory, 'corrupt-release');
	cpSync(config.release, corrupt, { recursive: true });
	writeFileSync(
		path.join(
			corrupt,
			readdirSync(corrupt).find((name) => name.endsWith('.png')),
		),
		Buffer.from([0]),
	);
	...
	assert.deepEqual(
		...
		previous,
		'Failed forced restore must preserve the complete previous target',
	);
	console.log(
		'[consumer] Artifact PASS: SDK/CLI publish, binaries, atlas RGBA, cross-package references, limited restore, validation and failed-force rollback',
	);
}
