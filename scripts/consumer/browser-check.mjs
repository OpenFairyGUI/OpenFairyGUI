...
try {
	...
	context.on('page', (page) => {
		...
		page.on('request', (request) => {
			if (!request.url().startsWith(`${host.url}/`)) errors.push(`Unexpected request: ${request.url()}`);
		});
	});
	...
	const transaction = {
		sessionId: await page.evaluate(() => example.sessionId),
		expectedRevision: initial.revision,
		operations: [{ kind: 'setDisplayNodeProps', selector: initial.selector, props: { text: 'Saved in Chromium' } }],
	};
	...
	assert.deepEqual(
		preview.data.impact.entities.find(
			(entry) =>
				entry.target.kind === 'displayNode' &&
				entry.target.selector.displayNodeId === initial.selector.displayNodeId,
		),
		{ target: { kind: 'displayNode', selector: initial.selector }, change: 'updated', fields: ['text'] },
	);
	assert.deepEqual(preview.data.impact.files, [
	...
	const denied = await page.evaluate(() =>
		example.runtime.saveSession({ sessionId: example.sessionId, expectedRevision: 1, targetPath: '../escape' }),
	);
	assert.equal(denied.error?.code, 'path_policy_violation');
	assert.deepEqual(await snapshot(page), before);
	assert.equal(
		await page.evaluate(() => example.runtime.getSession({ sessionId: example.sessionId }).data.dirty),
		true,
	);
	...
	const pixels = await page.evaluate(async () => {
		const image = await createImageBitmap(
			new Blob([await example.fileSystem.readFileRaw('openfairygui-example/assets/Main/pixel.png')], {
				type: 'image/png',
			}),
		);
		try {
	...
	writeFileSync(
		...
		JSON.stringify(
			{ browser: browser.version(), checks, validation, pixels, changedFiles: altered, errors },
			null,
			2,
		),
	);
	console.log(
		`[consumer] Browser PASS: Chromium ${browser.version()}, ${checks.length} real storage/safety checks; browser-evidence.json and browser-consumer.png`,
	);
} catch (error) {
	writeFileSync(
		'browser-evidence.json',
		JSON.stringify({ browser: browser?.version(), checks, errors, failure: error.stack ?? String(error) }, null, 2),
	);
