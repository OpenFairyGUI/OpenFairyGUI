...
for (const [extension, selected] of [
	['mts', entries],
	['cts', entries.filter((entry) => entry.conditions.require)],
]) {
	assert(selected.length > 0);
	writeFileSync(
		`entries.${extension}`,
		selected
			.map(({ specifier }, index) => `import * as entry${index} from '${specifier}'; void entry${index};`)
			.join('\n'),
	);
}
writeFileSync(
	'publish-result.mts',
	`import { publishNode, type PublishNodeOptions, type PublishNodeResult } from '@openfairygui/functions/node';
...
execFileSync(process.execPath, [path.join('node_modules/typescript', compilerBin), '-p', 'tsconfig.json'], {
	cwd: root,
	stdio: 'inherit',
	timeout: 60_000,
});
...
const browser = entries.filter(
	({ specifier }) => !specifier.startsWith('@openfairygui/mcp') && !specifier.endsWith('/node'),
);
...
writeFileSync(
	...
	main
		.map(
			({ specifier }, index) =>
				`import * as entry${index} from '${specifier}'; console.log(Object.keys(entry${index}));`,
		)
		.join('\n'),
);
...
console.log(
	`[consumer] Tooling PASS: strict NodeNext .mts/.cts declarations; ${browser.length} browser/Worker exports bundled without Node externals`,
);
