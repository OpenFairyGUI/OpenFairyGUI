...
async function serve(config) {
	...
	if (config.sessionId) {
		...
		if (config.task.id === 'missing-source-bytes')
			delete project.packages[0].resources.find((resource) => resource.id === 'payload').sourceBytes;
		data(
			runtime.openProjectSession({
				project,
				sessionId: config.sessionId,
				storage: { fileSystem, fairyPath: config.projectPath },
			}),
		);
		data(
	...
	function recordState(phase) {
		...
		if (!session.ok) {
			record({ type: 'session-state', phase, state: { error: session.error.code } });
			return;
		}
		...
		for (const pkg of outline.packages)
			for (const resource of pkg.resources) {
				...
				for (const node of resource.component.displayList)
					query({ kind: 'displayNode', selector: { ...selector, displayNodeId: node.id } });
				for (const controller of resource.component.controllers)
					query({ kind: 'controller', selector: { ...selector, controllerName: controller.name } });
				for (const transition of resource.component.transitions)
					query({ kind: 'transition', selector: { ...selector, transitionName: transition.name } });
			}
		record({
			type: 'session-state',
			phase,
			state: {
				...
				outline,
				entities,
				validation: data(runtime.validateSession({ sessionId: config.sessionId })),
			},
		});
	}
	let injected = false;
	...
	const transport = {
		async start() {
			...
			wire.onmessage = (message) => {
				pending = pending
							...
							if (definition?.backendMethod === 'openSession' && typeof input.projectPath === 'string') {
								try {
									denied = ![config.projectPath, path.dirname(config.projectPath)].includes(
										realpathSync(path.resolve(config.cwd, input.projectPath)),
									);
								} catch {
									denied = true;
								}
							...
							if (denied) {
								...
								await transport.send({
									...
									error: {
										code: -32602,
										message:
											'Evaluation host exposes only the supplied project and session-editing methods.',
									},
								});
								return;
							}
					...
					.catch((error) => {
						record({ type: 'host-error', message: error.message });
		...
		async send(message) {
			...
			if (request?.params?.name === toolName('closeSession') && result?.ok)
				sessions.delete(request.params.arguments.sessionId);
...
async function reference(config, configPath) {
	...
	const transport = new StdioClientTransport({
		...
		args: [fileURLToPath(import.meta.url), '--serve', configPath],
		cwd: root,
		stderr: 'pipe',
	});
	const client = new Client({ name: 'ofgui-eval-reference', version: '1' });
	const call = async (method, args) =>
		(await client.callTool({ name: toolName(method), arguments: args })).structuredContent.backendResult;
	try {
		...
		const opened = data(
			await call(
				config.sessionId ? 'getSession' : 'openSession',
				config.sessionId ? { sessionId: config.sessionId } : { projectPath: config.projectPath },
			),
		);
		...
		if (BLOCKERS[config.task.id]) {
			const result =
				config.task.id === 'path-policy'
					? await call('saveSession', {
							sessionId,
							expectedRevision: opened.revision,
							targetPath: config.targetPath,
						})
					: await call('preflightTransaction', {
							sessionId,
							expectedRevision: opened.revision,
							operations: [
								{
									kind: 'renameResource',
									selector: { packageId: 'pkgdemo1', resourceId: 'payload' },
									newName: 'RenamedPayload.bin',
								},
							],
						});
			assert(
				!result.ok &&
					result.meta.diagnostics.some((diagnostic) => diagnostic.code === BLOCKERS[config.task.id]),
			...
			if (['edit-display-node', 'compound-edit'].includes(config.task.id)) {
				...
				for (const node of resource.component.displayList.filter((node) => node.name === 'title')) {
					const candidate = {
						kind: 'displayNode',
						selector: { packageId: pkg.id, componentResourceId: resource.id, displayNodeId: node.id },
					};
					if (
						data(await call('queryEntity', { sessionId, target: candidate })).entity.properties.text ===
						'Hello OpenFairyGUI'
					)
			...
			if (config.task.id === 'edit-controller')
				target = {
					kind: 'controller',
					selector: { packageId: pkg.id, componentResourceId: resource.id, controllerName: 'state' },
				};
			if (config.task.id === 'edit-transition')
				target = {
					kind: 'transition',
			...
			if (target.kind === 'transition') {
				...
				Object.assign(items[0], { duration: 18, endValue: [120, 64] });
			...
			if (!applied.ok && applied.error.code === 'stale_write') {
				...
				assert.equal(
					refreshed.packages
						.find((entry) => entry.id === pkg.id)
						.resources.find((entry) => entry.id === resource.id).name,
					'MainView',
				);
		...
		saveJson(config.final, {
			summary: 'Deterministic harness self-check; not a model observation.',
			outcome: BLOCKERS[config.task.id] ? 'blocked' : 'completed',
			blocker: BLOCKERS[config.task.id] ?? null,
			facts: {
				packageCount: outline.packages.length,
				resourceCount: outline.packages.reduce((sum, pkg) => sum + pkg.resources.length, 0),
...
async function codex(config, configPath, options) {
	...
	writeFileSync(
		instructions,
		'You are an OpenFairyGUI consumer. Complete the user task using only the supplied MCP tools and installed documentation resources. Repository source, shell, internet, personal skills and other applications are not available. Explain any blocker honestly.\n',
	);
	const enabledTools =
		config.task.lane === 'artifact'
	...
	child.stdout.on('data', (chunk) => {
		stdout += chunk;
		appendFileSync(path.join(config.directory, 'agent.jsonl'), chunk);
	});
	child.stderr.on('data', (chunk) => {
		stderr += chunk;
		appendFileSync(path.join(config.directory, 'agent.stderr.txt'), chunk);
	});
	...
	const clientWarnings = [
		...new Set(
			stderr
				.split(/\r?\n/)
				.filter((line) => line.includes('Skipping MCP tool'))
				.map((line) => line.slice(line.indexOf('Skipping MCP tool'))),
		),
	];
	return {
		ok:
...
async function runCase(task, options) {
	if (task.lane === 'artifact')
		return runArtifactCase(task, options, (config, configPath) =>
			options.runner === 'reference' ? reference(config, configPath) : codex(config, configPath, options),
		);
		...
		component.controllers = [
			{
				...
				homePage: '0',
				pages: [
					{ id: '0', name: 'Idle', remark: 'Keep idle' },
					{ id: '1', name: 'Active', remark: 'Keep active' },
				],
				actions: [
					{
						name: 'play-intro',
		...
		component.transitions = [
			{
				...
				fps: 24,
				items: [
					{ ...structuredClone(item), name: 'keep', time: 24, label: 'keep-title', endLabel: 'keep-done' },
					item,
				],
			},
		];
		component.displayList[0].gears = [
	...
	if (task.id === 'edit-display-node' || task.id === 'missing-source-bytes') {
		...
		if (task.id === 'edit-display-node') {
			...
			distraction.id = 'other-title';
			distraction.text = 'Leave this title alone';
			distraction.position = { x: 16, y: 80 };
			...
			project.packages[0].resources.push({
				...
				branchItemIds: [],
				file: 'payload.bin',
				sourceBytes: new Uint8Array([0, 255, 42]),
			});
		await writeProjectFromUam(new NodeIO(), project, projectPath);
	}
	...
	if (task.id === 'path-policy') {
		mkdirSync(path.dirname(targetPath));
		writeFileSync(targetPath, 'Destination sentinel: must not be overwritten.\n');
	}
	...
	saveJson(path.join(directory, 'before.json'), { project: before, files: beforeFiles });
	...
	try {
		runner =
			options.runner === 'reference'
				? await reference(config, configPath)
				: await codex(config, configPath, options);
	} catch (failure) {
	...
	const grade = gradeEvaluation({
		...
		trace,
		final,
		runnerOk: runner.ok && !error && !trace.some((entry) => entry.type === 'host-error'),
...
async function main(options) {
	...
	for (const source of json(path.join(root, 'expected.json'))) {
		...
		exportFiles(directory, json(path.join(directory, 'package.json')));
...
try {
	if (process.argv[2] === '--serve') await serve(json(process.argv[3]));
		...
		await main({
			runner: 'reference',
			output: path.join(root, 'reference-evaluations'),
			tasks: json(path.join(root, 'evaluation-tasks.json')),
		});
