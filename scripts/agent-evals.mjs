...
export function evaluationOptions(values) {
	...
	assert(
		Number.isSafeInteger(timeoutSeconds) && timeoutSeconds > 0 && timeoutSeconds <= 1800,
		'--timeout-seconds must be an integer in 1..1800.',
	);
	assert(
		values.runner !== 'codex' || values.codex,
		'Supply --codex with an installed Codex executable (on Windows use codex.exe, not a .cmd shim).',
	);
	...
	try {
		...
		if (directory !== archives) {
			...
			for (const manifest of expected)
				cpSync(path.join(directory, artifactName(manifest)), path.join(archives, artifactName(manifest)));
		}
			...
			provenance: {
				...
				harness: Object.fromEntries(
					[
						...
						'scripts/consumer/agent-eval.mjs',
						'scripts/consumer/artifact-eval.mjs',
						'scripts/consumer/runtime.mjs',
						'scripts/consumer/helpers.mjs',
						'scripts/agent-eval-checks.mjs',
						'examples/create-demo-project.mjs',
						'examples/publish-restore/index.mjs',
						'agent/evals/tasks.json',
					].map((file) => [file, digest(path.join(ROOT, file))]),
				),
				artifacts: expected.map((manifest) => ({
					name: manifest.name,
					version: manifest.version,
					file: artifactName(manifest),
					sha256: digest(path.join(directory, artifactName(manifest))),
				})),
			},
		};
