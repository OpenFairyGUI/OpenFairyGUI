	...
	try {
		...
		const configuration = {
			...
			provenance: {
				...
				harness: Object.fromEntries(['scripts/agent-evals.mjs', 'scripts/pack-smoke.mjs', 'scripts/repo-utils.mjs', 'scripts/consumer/agent-eval.mjs', 'scripts/consumer/runtime.mjs', 'scripts/agent-eval-checks.mjs', 'examples/create-demo-project.mjs', 'agent/evals/tasks.json'].map((file) => [file, digest(path.join(ROOT, file))])),
