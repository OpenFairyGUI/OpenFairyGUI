...
export function inspectReferences(root) {
		...
		if (entry && expectedCommit && existsSync(path.join(root, config.path, '.git'))) {
			...
			status =
				actualCommit !== expectedCommit
					? 'mismatch'
					: !existsSync(path.join(root, config.path, entry.probe))
						? 'incomplete'
						: git(path.join(root, config.path), ['--no-optional-locks', 'status', '--porcelain']).trim()
							? 'dirty'
							: 'ready';
		}
	...
	for (const entry of manifest.submodules) {
		if (!entries.some((item) => item.path === entry.path))
			entries.push({ path: entry.path, status: 'unregistered' });
	}
	return {
...
export function inspectEnvironment(root, { nodeVersion, pnpmVersion }) {
	...
	return [
		...
		{
			id: 'development-node',
			status: major === Number(recommended) ? 'ok' : 'warning',
			recommended,
		...
		},
		{
			...
			actual: pnpmVersion ?? 'unavailable',
			required: expectedPnpm,
		},
	];
}
...
export function inspectBuilds(root) {
	...
	for (const entry of readdirSync(path.join(root, 'packages'), { withFileTypes: true })) {
		...
		const missing = [
			...new Set(exportPaths(manifest.exports ?? manifest.main).concat(exportPaths(manifest.bin))),
		...
		if (manifest.name === '@openfairygui/cli' && !existsSync(path.join(directory, 'dist/cli.mjs')))
			missing.push('dist/cli.mjs');
...
export async function doctor(root) {
	...
	if (!pnpmVersion) {
		try {
			...
			/* Report missing/unverifiable pnpm below; never install or change machine configuration. */
		}
	}
	...
	try {
		...
		for (const format of ['png', 'jpeg']) {
			const encoded = await sharp({ create: { width: 1, height: 1, channels: 3, background: '#204060' } })
				.toFormat(format)
				.toBuffer();
			const { data, info } = await sharp(encoded).raw().toBuffer({ resolveWithObject: true });
			if (info.width !== 1 || info.height !== 1 || info.channels !== 3 || data.length !== 3)
				throw new Error(`${format} pixel decoding returned unexpected dimensions.`);
		}
		checks.push({
			id: 'sharp',
			status: 'ok',
			version: sharp.versions.sharp,
			message: 'In-memory PNG/JPEG encoding and pixel decoding passed.',
		});
	} catch (error) {
		checks.push({
			id: 'sharp',
			status: 'warning',
			message: `Native image capability unavailable or failed; ask the host to check dependencies before image/publish/restore tasks. ${error.message}`,
		});
	}
	try {
		...
		checks.push({
			...
			path: tmpdir(),
			message:
				'Directory access flags only; doctor does not create files or prove ACL permissions, space or later writes.',
		});
	} catch (error) {
	...
	return {
		...
		references,
		next: references.ok
			? 'pnpm check:ci (full) or pnpm check:fast (impact-selected; not full)'
			: 'pnpm refs:status; pnpm refs:sync; pnpm refs:verify',
		boundary:
			'Export existence and in-memory PNG/JPEG only; no build freshness, publish/restore or runtime-rendering proof. Use pnpm pack:check for actual Node/Web consumer boundaries.',
	};
}
...
if (isMain(import.meta.url)) {
	...
	try {
		...
		if (positionals.length > 1 || (positionals[0] && positionals[0] !== 'refs'))
			throw new Error('Usage: repo-doctor.mjs [refs] [--json] [--strict]');
		...
		if (json) console.log(JSON.stringify(report, null, 2));
			...
			for (const check of report.checks ?? [])
				console.log(`${check.status}: ${check.id} ${JSON.stringify(check)}`);
			const refs = refsOnly ? report : report.references;
			for (const entry of refs.submodules)
				console.log(`${entry.status}: ${entry.path} (expected ${entry.expectedCommit ?? 'gitlink missing'})`);
			console.log(
				report.next ??
					'Sources: .gitmodules; pins: Git index gitlinks. Use pnpm refs:sync to initialize, never --remote.',
			);
		}
