...
export function generateContract(program = createContractProgram()) {
	...
	const methods = runtime.declaration.members.filter(
		(member) =>
	...
	const metadata = constantValue(
		checker,
		checker.getTypeOfSymbolAtLocation(metadataExport.symbol, metadataExport.declaration),
	);
	assert.deepEqual(
		metadata.map((entry) => entry.backendMethod),
		methods.map((method) => method.name.text),
		'MCP mapping must cover every public Backend method exactly once, in capability order',
	);
	...
	const omitted = constantValue(
		checker,
		checker.getTypeOfSymbolAtLocation(omittedExport.symbol, omittedExport.declaration),
	);
	for (const name of Object.keys(omitted))
		assert(
			methods.some((method) => method.name.text === name),
			`Stale MCP method omission: ${name}`,
		);
	...
	for (const [index, method] of methods.entries()) {
		...
		if (signature.parameters[0]) {
			...
			for (const name of omitted[method.name.text] ?? [])
				assert(
					fields.some((field) => field.name === name),
					`Stale MCP omission: ${method.name.text}.${name}`,
				);
			for (const field of fields) {
				if (omitted[method.name.text]?.includes(field.name)) continue;
				input.properties[field.name] = emitter.schema(
					checker.getTypeOfSymbolAtLocation(field, field.valueDeclaration ?? field.declarations[0]),
				);
	...
	assert.deepEqual(
		constantValue(checker, checker.getTypeOfSymbolAtLocation(methodsProperty, methodsProperty.valueDeclaration)),
		Object.keys(tools),
		'Backend capabilities must list exactly the public methods',
	);
	const versions = {};
	...
	assert.equal(
		new Set(snapshot.diagnostics.map((guide) => guide.code)).size,
		snapshot.diagnostics.length,
		'Duplicate diagnostic recovery guide',
	);
	const literals = (type) =>
		(type.isUnion() ? type.types : [type]).map((part) => {
	...
	assert.deepEqual(
		snapshot.diagnostics.map((guide) => guide.code).sort(),
		formalCodes.sort(),
		'Recovery guides must cover every formal diagnostic code exactly once',
	);
	const transaction = new Set([
		...codes(CORE, 'UamTransactionErrorCode'),
		...codes(CORE, 'UamTransactionSupportIssueCode'),
	]);
	...
	const backend = new Set(
		literals(checker.getTypeOfPropertyOfType(backendError, 'code')).filter((code) => !transaction.has(code)),
	);
	for (const guide of snapshot.diagnostics) {
		const owners = [
			...
			.filter(([, codes]) => codes.has(guide.code))
			.map(([owner]) => owner);
		...
		if (values.probe) {
			console.log(
				JSON.stringify({
					...
					definitions: Object.keys(contract.$defs).length,
				}),
			);
		} else {
			...
			console.log(
				`Contract ${values.check ? 'check' : 'generation'} passed: ${Object.keys(contract.operations).length} operations / ${Object.keys(contract.tools).length} methods`,
			);
		}
	} catch (error) {
