			...
			resources: [
				{
					...
					name: 'MainView',
					kind: 'component',
					component: { displayList: [{ id: 'title', text: 'Original' }] },
				},
			],
		},
	],
	...
	return [
		{
			at: 10,
			type: 'request',
			message: { id, method: 'tools/call', params: { name: `openfairygui_backend_${method}`, arguments: args } },
		},
		{
			at: 20,
			type: 'response',
			message: { id, result: { isError: !result.ok, structuredContent: { backendResult: result } } },
		},
	];
}
function input(taskId = 'rename-save') {
	...
	return {
		taskId,
		expected: project,
		actual: structuredClone(project),
		expectedFiles: { 'package.xml': 'expected', 'notes.txt': 'untouched' },
		actualFiles: { 'package.xml': 'expected', 'notes.txt': 'untouched' },
		trace: [
...
test('eval oracles require real state, exact bytes and a saved result, not a completion claim or a prescribed tool order', () => {
	...
	assert(
		!gradeEvaluation({
			...inspect,
			final: { facts: { packageCount: 2, resourceCount: 1, validationStatus: 'valid' } },
		}).passed,
	);
...
test('stale evaluation binds rejection to the injected concurrent write and preserves its data', () => {
	...
	valid.trace.push(
		{ type: 'injection', ok: true, requestId: 3, previousRevision: 0, revision: 1 },
		...call(3, 'apply_transaction', { expectedRevision: 0 }, { ok: false, error: { code: 'stale_write' } }),
	);
	...
	assert(
		!gradeEvaluation({
			...valid,
			trace: valid.trace.map((entry) => (entry.type === 'injection' ? { ...entry, requestId: 99 } : entry)),
		}).passed,
	);
});
...
test('node editing oracle changes only the selected ID, text and position', () => {
	...
	fixture.packages[0].resources[0].component.displayList.unshift({
		id: 'other-title',
		name: 'title',
		text: 'Untouched',
		position: { x: 0, y: 1 },
	...
	assert.deepEqual(
		expected.packages[0].resources[0].component.displayList[0],
		fixture.packages[0].resources[0].component.displayList[0],
	);
	assert.deepEqual(expected.packages[0].resources[0].component.displayList[1], {
		id: 'title',
		text: 'Ready to edit',
		position: { x: 40, y: 56 },
	});
	...
	for (const mutate of [
		(project) => {
			project.packages[0].resources[0].component.displayList[0].text = 'Ready to edit';
		},
		(project) => {
			project.packages[0].resources[0].component.displayList[1].position.y = 57;
		},
		(project) => {
			project.packages[0].resources[0].id = 'replaced';
		},
	]) {
...
test('safe-stop grading requires a real diagnostic, intact live work and untouched files, never a claimed success', () => {
	for (const [taskId, code] of Object.entries(BLOCKERS)) {
		const state = {
			session: { sessionId: 'pending', revision: 1, lastSavedRevision: 0, dirty: true },
			entities: [{ text: 'Unsaved' }],
			validation: { status: taskId === 'missing-source-bytes' ? 'incomplete' : 'valid' },
		};
		const valid = {
			...input(taskId),
			final: { outcome: 'blocked', blocker: code },
			trace: [
				{ type: 'session-state', phase: 'before', state },
				...call(1, 'validate_session', {}, { ok: true, data: state.validation }),
				...call(
					2,
					taskId === 'path-policy' ? 'save_session' : 'preflight_transaction',
					{},
					{ ok: false, meta: { diagnostics: [{ code }] } },
				),
		...
		assert(gradeEvaluation(valid).passed);
		for (const broken of [
			{ final: { outcome: 'completed', blocker: null } },
			{ final: { outcome: 'blocked', blocker: 'made_up' } },
			...
			{ actualFiles: { ...valid.actualFiles, 'outside.txt': 'unexpected write' } },
			{
				trace: [
					...valid.trace,
					{ type: 'session-state', phase: 'after', state: { error: 'session_not_found' } },
				],
			},
			{
				trace: [
					...valid.trace,
					{
						type: 'session-state',
						phase: 'after',
						state: { ...state, session: { ...state.session, revision: 2 } },
					},
				],
			},
			...
			...['apply_transaction', 'save_session', 'close_session'].map((method) => ({
				trace: [...valid.trace, ...call(3, method, {}, { ok: true, data: { dirty: false } })],
			})),
		])
...
test('complex editing oracles preserve page IDs, actions, item order, targets and gears, and require the real query', () => {
	...
	component.controllers = [
		{
			...
			pages: [
				...
				{ id: '1', name: 'Active', remark: 'Keep too' },
			],
			actions: [{ toPageIds: ['1'], transitionName: 'intro' }],
		},
	];
	component.transitions = [
		{
			name: 'intro',
			items: [
				{ label: 'keep', targetNodeId: 'title' },
				{ label: 'move-title', targetNodeId: 'title', duration: 12, endValue: ['96', '48'] },
			],
		},
	];
	...
	for (const taskId of ['edit-controller', 'edit-transition']) {
		...
		const target = {
			...
			selector: {
				packageId: 'pkgdemo1',
				componentResourceId: 'cmpdemo1',
				[kind === 'controller' ? 'controllerName' : 'transitionName']:
					kind === 'controller' ? 'state' : 'intro',
			},
		};
		...
		for (const mutate of [
			(model) => {
				model.controllers[0].pages[1].id = 'replaced';
			},
			(model) => {
				model.controllers[0].pages[0].remark = '';
			},
			(model) => {
				model.controllers[0].actions[0].toPageIds = [];
			},
			(model) => {
				model.transitions[0].items.reverse();
			},
			(model) => {
				model.transitions[0].items[1].targetNodeId = '';
			},
			(model) => {
				model.displayList[0].gears = [];
			},
		]) {
			const actual = structuredClone(expected);
			mutate(actual.packages[0].resources[0].component);
			assert(!gradeEvaluation({ ...valid, actual }).passed);
		}
		const wrongScope = structuredClone(valid);
		wrongScope.trace.at(
			-1,
		).message.result.structuredContent.backendResult.data.target.selector.componentResourceId = 'cmpother';
...
test('observations count failures, docs, previews and exact resubmissions separately from success', () => {
	...
	const trace = [
		...
		...call(3, 'preflight_transaction', args, { ok: true }),
		{
			at: 1,
			type: 'request',
			message: { id: 4, method: 'resources/read', params: { uri: 'openfairygui://docs/workflow' } },
		},
			...
			message: {
				...
				result: {
					tools: [
						{
							name: 'example',
							inputSchema: { type: 'object', description: '字节' },
	...
	assert.equal(metrics.failedCalls, 2);
	assert.equal(metrics.staleRejections, 1);
	assert.equal(metrics.repeatedApplyArguments, 1);
	assert.equal(metrics.previews, 1);
	assert.deepEqual(metrics.documentationReads, ['openfairygui://docs/workflow']);
	assert.deepEqual(metrics.discoveries, [
		{
			tools: [
				{
					name: 'example',
					inputBytes: Buffer.byteLength(JSON.stringify({ type: 'object', description: '字节' })),
					outputBytes: 17,
				},
			],
			inputBytes: 40,
...
test('evaluation host rejects realpath escapes, including staged callbacks, before invoking filesystem writes', async () => {
	...
	let writes = 0;
	const base = {
		resolvePath: async (file) => (file.endsWith('link') ? outside : path.resolve(file)),
		writeFile: async () => {
			writes++;
		},
	};
	...
	await assert.rejects(
		fs.runProjectWriteTransaction(workspace, (staged) => staged.writeFile(outside, 'no')),
		/scope violation/,
	);
	assert.equal(writes, 1);
	assert.equal(records.filter((entry) => entry.type === 'scope-violation').length, 3);
});
...
test('manual model execution is explicit, bounded, tool-only and shell-free', () => {
	...
	for (const feature of ['shell_tool', 'unified_exec', 'plugins', 'apps', 'memories', 'multi_agent', 'hooks'])
	...
	assert(
		isolatedCodexEvents([
			{ item: { type: 'mcp_tool_call', server: 'codex', tool: 'list_mcp_resources', arguments: {} } },
		]),
	);
...
test('artifact host never widens editing tools or accepts arbitrary execution inputs', () => {
	...
	for (const tool of ARTIFACT_TOOLS)
		assertArtifactTool('publish-consume', tool.name, tool.name.endsWith('_inspect') ? { target: 'published' } : {});
	for (const [name, args] of [
		['ofgui_artifact_publish', { output: '../escape' }],
		['ofgui_artifact_restore', { force: true }],
		['ofgui_artifact_context', { command: 'anything' }],
		['ofgui_artifact_inspect', { target: '../escape' }],
		['ofgui_artifact_inspect', { target: 'published', path: '/other' }],
...
test('artifact grading needs real commands, decoded pixels, supported semantics and untouched inputs', () => {
	...
	for (const [index, [name, args]] of [
		['publish', {}],
		['restore', {}],
		['inspect', { target: 'published' }],
		['inspect', { target: 'restored' }],
	].entries()) {
		trace.push(
			{
				type: 'request',
				message: {
					id: index,
					method: 'tools/call',
					params: { name: `ofgui_artifact_${name}`, arguments: args },
				},
			},
			{
				type: 'response',
				message: { id: index, result: { structuredContent: { artifactResult: { success: true } } } },
			},
		);
	}
	const valid = {
		expected,
		actual: {
			published: { semantics: expected, pixelsMatch: true },
			restored: { semantics: expected, pixelsMatch: true, validation: { status: 'valid', complete: true } },
			exactOutputFiles: true,
		},
		beforeFiles: { 'source/keep': 'keep' },
		actualFiles: { 'source/keep': 'keep', 'release/package.fui': 'binary', 'restored/project.fairy': 'project' },
		trace,
		final: {
			outcome: 'completed',
			blocker: null,
			facts: { packageCount: 1, resourceCount: 2, validationStatus: 'valid' },
		},
		runnerOk: true,
		isolated: true,
		publishTask: true,
	...
	for (const broken of [
		...
		{ final: null },
		{
			trace: trace.map((entry) =>
				entry.type === 'response'
					? { ...entry, message: { ...entry.message, result: { ...entry.message.result, isError: true } } }
					: entry,
			),
		},
		...
		{ actual: { ...valid.actual, restored: { ...valid.actual.restored, semantics: [] } } },
		{
			actual: {
				...valid.actual,
				restored: { ...valid.actual.restored, validation: { status: 'valid', complete: false } },
			},
		},
	])
	...
	const restoreOnly = {
		...valid,
		publishTask: false,
		beforeFiles: { ...valid.beforeFiles, 'release/package.fui': 'binary' },
		trace: trace.filter((entry) => entry.message.id !== 0),
	};
	assert(gradeArtifactEvaluation(restoreOnly).passed);
	assert(
		!gradeArtifactEvaluation({
			...restoreOnly,
			actualFiles: { ...valid.actualFiles, 'release/package.fui': 'replaced' },
		}).passed,
	);
});
