...
export function selectTests(map, files, available, fallbackReason) {
	...
	if (files.length === 0 && !reason)
		reason = 'No changes found; run the full suite rather than report an empty test success.';
	...
	if (reason && selected.size !== available.length)
		throw new Error('Full fallback does not cover every test file. Update the impact map.');
	return {
...
export function impactTable(map) {
	return [
		...
		...map.rules.map(
			(rule) =>
				`| ${rule.paths.map((value) => `\`${value}\``).join(', ')} | ${rule.tests.join(', ') || '仅仓库检查'} | ${rule.docs.map((value) => `\`${value}\``).join(', ')} |`,
		),
	].join('\n');
}
...
if (isMain(import.meta.url)) {
	try {
		...
		if (values.check && !values.matrix && !values.base && !process.env.GITHUB_BASE_REF) {
			throw new Error(
				'check:fast requires --base <PR target>, for example --base origin/next. Add --list to preview the same plan without running checks.',
			);
		}
		...
		if (values.matrix) console.log(impactTable(map));
			...
			if (!values.list) {
				if (values.check) {
					for (const command of ['lint:ci', 'typecheck'])
						runCommand(ROOT, ...pnpmInvocation(process.env.npm_execpath, [command]));
				}
				...
				if (plan.tests.length > 0) {
					...
					if (!refs.ok)
						throw new Error(
							'Required fixtures are not ready. Run pnpm refs:status, then pnpm refs:sync / pnpm refs:verify.',
						);
