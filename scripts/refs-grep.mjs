...
export function grepReferences(root, pattern) {
	if (typeof pattern !== 'string' || !pattern || /[\r\n\0]/.test(pattern))
		throw new Error('Expected one non-empty, single-line literal search string.');
	const references = inspectReferences(root);
	if (!references.ok)
		throw new Error(
			'Required references are not ready. Run pnpm refs:status and reconcile missing, dirty or mismatched fixtures before searching.',
		);
	const lines = references.submodules.flatMap((entry) => {
		let output;
		try {
			output = git(path.join(root, entry.path), [
				'--no-optional-locks',
				'-c',
	...
	if (Buffer.byteLength(lines.join('\n'), 'utf8') > 64 * 1024)
		throw new Error(
			'Reference matches exceed 64 KiB; narrow the literal or use native Git/rg in a verified source directory. No partial results returned.',
		);
	return lines;
}
