...
export function consumerEnvironment(environment = process.env) {
	return Object.fromEntries(
		Object.entries(environment).filter(
			([key]) =>
				!/^(NODE_PATH|NODE_OPTIONS|TSX_TSCONFIG_PATH|TS_NODE_PROJECT|TS_NODE_COMPILER_OPTIONS|OPENFAIRYGUI_ALLOWED_PROJECT_ROOTS)$/i.test(
					key,
				),
		),
	);
...
export function preparePackedConsumer({ artifacts } = {}) {
	...
	const command = (cwd, binary, args) =>
		runCommand(cwd, binary, args, {
			env,
			encoding: 'utf8',
	...
	try {
		...
		if (!artifacts) {
			...
			for (const manifest of expected) {
				...
				pnpm(path.join(ROOT, manifest.repository.directory), [
					'pack',
					'--out',
					path.join(directory, artifactName(manifest)),
				]);
			}
		}
		const dependencies = {
			...readJson(path.join(ROOT, 'examples/package.json')).dependencies,
			...Object.fromEntries(
				expected.map((manifest) => {
		...
		mkdirSync(consumer);
		json(path.join(consumer, 'package.json'), {
			name: 'ofgui-isolated-consumer',
			private: true,
			type: 'module',
			dependencies,
		...
		for (const name of [
			...
			cpSync(path.join(ROOT, 'scripts/consumer', name), path.join(consumer, name));
		...
		cpSync(path.join(ROOT, 'examples'), path.join(consumer, 'examples'), {
			...
			filter: (file) =>
				!file
					.split(path.sep)
					.some(
						(part) => part === 'node_modules' || part === 'package-lock.json' || part === 'pnpm-lock.yaml',
					),
		});
...
export function packSmoke({ artifacts, keep = false, 'browser-deps': browserDeps = false } = {}) {
	...
	try {
		...
		const devDependencies = {
			...
			zod: createRequire(realpathSync(path.join(consumer, 'node_modules/@openfairygui/mcp/package.json')))(
				'zod/package.json',
			).version,
		...
		pnpm(consumer, [
			...
			'chromium',
			'--only-shell',
			...(browserDeps && process.platform === 'linux' ? ['--with-deps'] : []),
		]);
		console.log(command(consumer, process.execPath, ['browser-check.mjs']).trim());
		console.log(
			'[consumer] PASS: five tarballs, Node ESM/CJS, types, real Chromium storage, CLI, MCP, examples and deterministic evaluation checks',
		);
		passed = true;
	} finally {
...
if (isMain(import.meta.url)) {
	try {
		const { values } = parseArgs({
			options: { artifacts: { type: 'string' }, keep: { type: 'boolean' }, 'browser-deps': { type: 'boolean' } },
		});
		packSmoke(values);
	} catch (error) {
