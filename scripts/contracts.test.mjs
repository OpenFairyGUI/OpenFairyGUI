...
test('CLI schemas cover every registered command and reject malformed envelopes; new diagnostic codes fail closed', async () => {
	...
	for (const [command, output] of Object.entries(contract.cli)) {
		assert(
			z.fromJSONSchema({ ...output, $defs: contract.$defs }).safeParse({
				schemaVersion: 1,
				command,
				success: false,
				error: { code: 'invalid_arguments', message: 'Invalid arguments' },
			}).success,
			`CLI failure schema: ${command}`,
		);
	}
	...
	const report = {
		...
		command: 'validate',
		success: true,
		result: { status: 'valid', complete: true, diagnostics: [] },
	};
	...
	assert(
		!schema.safeParse({
			...
			result: {
				...report.result,
				diagnostics: [{ code: 'invented', message: 'x', severity: 'error', path: '' }],
			},
		}).success,
	);
	...
	assert.throws(
		() =>
			generateContract(
				createContractProgram(ROOT, {
					[file]: before.replace("| 'invalid_project_xml'", "| 'contract_probe' | 'invalid_project_xml'"),
				}),
			),
		/every formal diagnostic code/,
	);
	const cli = 'packages/cli/src/contracts.ts';
	const source = readFileSync(path.join(ROOT, cli), 'utf8');
	const changed = generateContract(
		createContractProgram(ROOT, { [cli]: source.replace('schemaVersion: 1;', 'schemaVersion: 2;') }),
	);
...
test('canonical types generate precise schemas for the four representative operations', async () => {
	...
	const update = {
		...
		props: { text: 'ok', position: { x: 1, y: 2 } },
	};
	...
	assert(
		!schema(add.kind).safeParse({
			...add,
			component: {
				...component,
				component: { ...component.component, displayList: [{ kind: 'text', text: 'incomplete' }] },
			},
		}).success,
	);
	const replace = {
		kind: 'replaceResourceBytes',
		selector: { packageId: 'p', resourceId: 'r' },
		sourceBytes: [0, 255],
	};
	...
	assert(
		schema('addResourceFolder').safeParse({
			kind: 'addResourceFolder',
			selector: { packageId: 'p' },
			path: '/icons/',
			branch: '',
		}).success,
	);
	assert(
		schema('removeResourceFolder').safeParse({
			kind: 'removeResourceFolder',
			selector: { packageId: 'p', path: '/icons/', branch: '' },
		}).success,
	);
...
test('current snapshot and bilingual tables match; a canonical field change fails drift checks without editing files', () => {
	...
	assert.throws(
		() => checkGeneratedFiles({ 'docs/guide/contracts.md': files['docs/guide/contracts.md'] }),
		/Generated contract drift/,
	);
	assert.throws(
		() => checkGeneratedFiles({ 'docs/en/guide/contracts.md': files['docs/en/guide/contracts.md'] }),
		/Generated contract drift/,
	);
	...
	const extraMethod = backend.replace(
		'public getCapabilities()',
		'public contractProbe() { return this.getCapabilities(); }\n\tpublic getCapabilities()',
	);
	assert.notEqual(extraMethod, backend);
	assert.throws(
		() => generateContract(createContractProgram(ROOT, { [backendFile]: extraMethod })),
		/MCP mapping must cover every public Backend method/,
	);
});

test('installed documentation is bound to the package version, source text and generated contract', async () => {
	const { getInstalledDocumentationIndex, readInstalledDocumentation, getInstalledContractSnapshot } = await tsImport(
		'../packages/backend/src/docs.ts',
		import.meta.url,
	);
	...
	assert.equal(
		readInstalledDocumentation('workflow').text,
		readFileSync(path.join(ROOT, 'packages/backend/docs/workflow.md'), 'utf8').replaceAll('\r\n', '\n'),
	);
	assert.equal(
		readInstalledDocumentation('restore-limits').text,
		readFileSync(path.join(ROOT, 'docs/published-project-restore-limitations.md'), 'utf8').replaceAll('\r\n', '\n'),
	);
	...
	assert.throws(
		() => checkGeneratedFiles({ [docsFile]: files[docsFile].replace(manifest.version, '0.0.0-drift') }),
		/Generated contract drift/,
	);
});
...
test('generated byte paths cover declared resource snapshots and reject unsupported recursive native bytes', async () => {
	...
	const recursive = {
		type: 'object',
		properties: { next: { $ref: '#/$defs/Node' }, bytes: { 'x-openfairygui-native': 'Uint8Array' } },
	};
