...
> 将授权工程中指定组件的标题改为“开始游戏”，保持其他内容不变。先查询并预演，再提交、验证、保存并回读；遇到目标不唯一、revision 冲突或验证不完整时停止并报告。

[接入指南](./docs/guide/getting-started.md#完成首个编辑任务)提供具体目标、示例工程与完整验收步骤；[可运行示例](./docs/guide/examples.md)已纳入安装包消费者验证。
